...
public class ComputeInField {
    ...
    /* renamed from: A */
    private short[][] f8054A;

    /* renamed from: x */
    short[] f8055x;

    private void computeZerosAbove() throws RuntimeException {
        for (int length = this.f8054A.length - 1; length > 0; length--) {
            for (int i = length - 1; i >= 0; i--) {
                short[][] sArr = this.f8054A;
    ...
    */
    private void computeZerosUnder(boolean z) throws RuntimeException {
        int length = z ? this.f8054A.length * 2 : this.f8054A.length + 1;
        int i = 0;
        while (i < this.f8054A.length - 1) {
            int i2 = i + 1;
            int i3 = i2;
            while (true) {
                short[][] sArr = this.f8054A;
                if (i3 < sArr.length) {
                    ...
                    for (int i4 = i; i4 < length; i4++) {
                        short sMultElem = GF2Field.multElem(s, GF2Field.multElem(this.f8054A[i][i4], sInvElem));
                        short[] sArr2 = this.f8054A[i3];
    ...
    private void substitute() throws IllegalStateException {
        short sInvElem = GF2Field.invElem(this.f8054A[r0.length - 1][r0.length - 1]);
        if (sInvElem == 0) {
            throw new IllegalStateException("The equation system is not solvable");
        }
        short[] sArr = this.f8055x;
        short[][] sArr2 = this.f8054A;
        sArr[sArr2.length - 1] = GF2Field.multElem(sArr2[sArr2.length - 1][sArr2.length], sInvElem);
        for (int length = this.f8054A.length - 2; length >= 0; length--) {
            short[][] sArr3 = this.f8054A;
            short sAddElem = sArr3[length][sArr3.length];
            for (int length2 = sArr3.length - 1; length2 > length; length2--) {
                sAddElem = GF2Field.addElem(sAddElem, GF2Field.multElem(this.f8054A[length][length2], this.f8055x[length2]));
            }
            short sInvElem2 = GF2Field.invElem(this.f8054A[length][length]);
            if (sInvElem2 == 0) {
                throw new IllegalStateException("Not solvable equation system");
            }
            this.f8055x[length] = GF2Field.multElem(sAddElem, sInvElem2);
        }
    }
    ...
    public short[][] inverse(short[][] sArr) {
        ...
        try {
            int i = 0;
            this.f8054A = (short[][]) Array.newInstance((Class<?>) cls, sArr.length, sArr.length * 2);
            if (sArr.length != sArr[0].length) {
                throw new RuntimeException("The matrix is not invertible. Please choose another one!");
            }
            for (int i2 = 0; i2 < sArr.length; i2++) {
                for (int i3 = 0; i3 < sArr.length; i3++) {
                    this.f8054A[i2][i3] = sArr[i2][i3];
                }
                for (int length = sArr.length; length < sArr.length * 2; length++) {
                    this.f8054A[i2][length] = 0;
                }
                short[][] sArr2 = this.f8054A;
    ...
    public short[][] multiplyMatrix(short[][] sArr, short[][] sArr2) throws RuntimeException {
        ...
        this.f8054A = (short[][]) Array.newInstance((Class<?>) Short.TYPE, sArr.length, sArr2[0].length);
        for (int i = 0; i < sArr.length; i++) {
            for (int i2 = 0; i2 < sArr2.length; i2++) {
    ...
    public short[] solveEquation(short[][] sArr, short[] sArr2) {
        ...
        try {
            this.f8054A = (short[][]) Array.newInstance((Class<?>) Short.TYPE, sArr.length, sArr.length + 1);
            this.f8055x = new short[sArr.length];
            for (int i = 0; i < sArr.length; i++) {
                for (int i2 = 0; i2 < sArr[0].length; i2++) {
                    this.f8054A[i][i2] = sArr[i][i2];
                }
            }
            for (int i3 = 0; i3 < sArr2.length; i3++) {
                short[] sArr3 = this.f8054A[i3];
