...
public class McElieceCipher implements MessageEncryptor {
    ...
    private GF2Vector computeMessageRepresentative(byte[] bArr) {
        byte[] bArr2 = new byte[this.maxPlainTextSize + ((this.f8007k & 7) != 0 ? 1 : 0)];
        System.arraycopy(bArr, 0, bArr2, 0, bArr.length);
        bArr2[bArr.length] = 1;
        return GF2Vector.OS2VP(this.f8007k, bArr2);
    }

    private void initCipherDecrypt(McEliecePrivateKeyParameters mcEliecePrivateKeyParameters) {
        this.f8008n = mcEliecePrivateKeyParameters.getN();
        int k = mcEliecePrivateKeyParameters.getK();
        this.f8007k = k;
        this.maxPlainTextSize = k >> 3;
        this.cipherTextSize = this.f8008n >> 3;
    }

    private void initCipherEncrypt(McEliecePublicKeyParameters mcEliecePublicKeyParameters) {
        this.f8008n = mcEliecePublicKeyParameters.getN();
        this.f8007k = mcEliecePublicKeyParameters.getK();
        this.f8010t = mcEliecePublicKeyParameters.getT();
        this.cipherTextSize = this.f8008n >> 3;
        this.maxPlainTextSize = this.f8007k >> 3;
    }
    ...
    public void init(boolean z, CipherParameters cipherParameters) {
        ...
        if (!z) {
            ...
            if (!(cipherParameters instanceof ParametersWithRandom)) {
                this.f8009sr = CryptoServicesRegistrar.getSecureRandom();
            ...
            this.f8009sr = parametersWithRandom.getRandom();
    ...
    public byte[] messageDecrypt(byte[] bArr) throws InvalidCipherTextException {
        ...
        GF2Vector gF2VectorOS2VP = GF2Vector.OS2VP(this.f8008n, bArr);
        ...
        return computeMessage((GF2Vector) sInv.leftMultiply(gF2Vector2.extractRightVector(this.f8007k)));
    }
    ...
    public byte[] messageEncrypt(byte[] bArr) {
        ...
        return ((GF2Vector) ((McEliecePublicKeyParameters) this.key).getG().leftMultiply(gF2VectorComputeMessageRepresentative).add(new GF2Vector(this.f8008n, this.f8010t, this.f8009sr))).getEncoded();
    }
}
