...
class WotsPlus {
    ...
    WotsPlus(SPHINCSPlusEngine sPHINCSPlusEngine) {
        ...
        this.f8067w = sPHINCSPlusEngine.WOTS_W;
    }
    ...
    byte[] chain(byte[] bArr, int i, int i2, byte[] bArr2, ADRS adrs) {
        ...
        return this.engine.mo3977F(bArr2, adrs, bArrChain);
    }

    public byte[] pkFromSig(byte[] bArr, byte[] bArr2, byte[] bArr3, ADRS adrs) {
        SPHINCSPlusEngine sPHINCSPlusEngine;
        ADRS adrs2 = new ADRS(adrs);
        int[] iArrBase_w = base_w(bArr2, this.f8067w, this.engine.WOTS_LEN1);
        int i = 0;
        int i2 = 0;
        while (true) {
            ...
            i2 += (this.f8067w - 1) - iArrBase_w[i];
            i++;
        }
        int i3 = sPHINCSPlusEngine.WOTS_LEN2;
        int i4 = sPHINCSPlusEngine.WOTS_LOGW;
        int[] iArrConcatenate = Arrays.concatenate(iArrBase_w, base_w(Arrays.copyOfRange(Pack.intToBigEndian(i2 << (8 - ((i3 * i4) % 8))), 4 - (((i3 * i4) + 7) / 8), 4), this.f8067w, this.engine.WOTS_LEN2));
        SPHINCSPlusEngine sPHINCSPlusEngine2 = this.engine;
        byte[] bArr4 = new byte[sPHINCSPlusEngine2.f8062N];
        byte[][] bArr5 = new byte[sPHINCSPlusEngine2.WOTS_LEN][];
        for (int i5 = 0; i5 < this.engine.WOTS_LEN; i5++) {
            adrs.setChainAddress(i5);
            int i6 = this.engine.f8062N;
            System.arraycopy(bArr, i5 * i6, bArr4, 0, i6);
            int i7 = iArrConcatenate[i5];
            bArr5[i5] = chain(bArr4, i7, (this.f8067w - 1) - i7, bArr3, adrs);
        }
        adrs2.setType(1);
    ...
    byte[] pkGen(byte[] bArr, byte[] bArr2, ADRS adrs) {
        ...
        while (i < this.engine.WOTS_LEN) {
            ...
            byte[] bArr4 = bArr2;
            bArr3[i] = chain(this.engine.PRF(bArr, adrs3), 0, this.f8067w - 1, bArr4, adrs3);
            i++;
            bArr2 = bArr4;
        }
        adrs2.setType(1);
    ...
    public byte[] sign(byte[] bArr, byte[] bArr2, byte[] bArr3, ADRS adrs) {
        SPHINCSPlusEngine sPHINCSPlusEngine;
        ADRS adrs2 = new ADRS(adrs);
        int[] iArrBase_w = base_w(bArr, this.f8067w, this.engine.WOTS_LEN1);
        int i = 0;
        int i2 = 0;
        while (true) {
            sPHINCSPlusEngine = this.engine;
        ...
        int[] iArrConcatenate = Arrays.concatenate(iArrBase_w, base_w(Arrays.copyOfRange(bArrIntToBigEndian, i4, bArrIntToBigEndian.length), this.f8067w, this.engine.WOTS_LEN2));
        ...
        while (i5 < this.engine.WOTS_LEN) {
            ...
            byte[] bArr5 = bArr3;
            bArr4[i5] = chain(this.engine.PRF(bArr2, adrs2), 0, iArrConcatenate[i5], bArr5, adrs2);
            i5++;
            bArr3 = bArr5;
        }
