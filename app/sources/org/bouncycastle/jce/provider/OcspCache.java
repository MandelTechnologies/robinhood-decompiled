...
class OcspCache {
    ...
    private static Map<URI, WeakReference<Map<CertID, OCSPResponse>>> f10850cache = Collections.synchronizedMap(new WeakHashMap());

    OcspCache() {
    }

    /* JADX WARN: Multi-variable type inference failed */
    static OCSPResponse getOcspResponse(CertID certID, PKIXCertRevocationCheckerParameters pKIXCertRevocationCheckerParameters, URI uri, X509Certificate x509Certificate, List<Extension> list, JcaJceHelper jcaJceHelper) throws IOException, CertPathValidatorException {
        ...
        WeakReference<Map<CertID, OCSPResponse>> weakReference = f10850cache.get(uri);
        ...
        try {
            ...
            try {
                ...
                WeakReference<Map<CertID, OCSPResponse>> weakReference2 = f10850cache.get(uri);
                if (weakReference2 != null) {
                    weakReference2.get().put(certID, oCSPResponse);
                } else {
                    HashMap map2 = new HashMap();
                    map2.put(certID, oCSPResponse);
                    f10850cache.put(uri, new WeakReference<>(map2));
                }
                return oCSPResponse;
