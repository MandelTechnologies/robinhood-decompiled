...
public class KDFFeedbackBytesGenerator implements MacDerivationFunction {
    ...
    public KDFFeedbackBytesGenerator(Mac mac) {
        ...
        this.f7554h = macSize;
        this.f7556k = new byte[macSize];
    }

    private void generateNext() throws IllegalStateException, DataLengthException {
        if (this.generatedBytes == 0) {
            Mac mac = this.prf;
            byte[] bArr = this.f7555iv;
            mac.update(bArr, 0, bArr.length);
        } else {
            Mac mac2 = this.prf;
            byte[] bArr2 = this.f7556k;
            mac2.update(bArr2, 0, bArr2.length);
        }
        if (this.useCounter) {
            int i = (this.generatedBytes / this.f7554h) + 1;
            byte[] bArr3 = this.ios;
        ...
        this.prf.doFinal(this.f7556k, 0);
    }
    ...
    public int generateBytes(byte[] bArr, int i, int i2) throws IllegalStateException, DataLengthException, IllegalArgumentException {
        ...
        System.arraycopy(this.f7556k, i7, bArr, i, iMin);
        ...
        while (true) {
            ...
            generateNext();
            iMin = Math.min(this.f7554h, i8);
            System.arraycopy(this.f7556k, 0, bArr, i, iMin);
            this.generatedBytes += iMin;
            i8 -= iMin;
        }
    }
    ...
    public void init(DerivationParameters derivationParameters) throws IllegalArgumentException {
        ...
        if (kDFFeedbackParameters.useCounter()) {
            BigInteger bigIntegerMultiply = TWO.pow(r).multiply(BigInteger.valueOf(this.f7554h));
            if (bigIntegerMultiply.compareTo(INTEGER_MAX) != 1) {
                iIntValue = bigIntegerMultiply.intValue();
            }
        }
        this.maxSizeExcl = iIntValue;
        this.f7555iv = kDFFeedbackParameters.getIV();
