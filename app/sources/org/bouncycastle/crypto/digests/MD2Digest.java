...
public class MD2Digest implements ExtendedDigest, Memoable {
    ...
    public MD2Digest() {
        this.f7326X = new byte[48];
        this.f7325M = new byte[16];
        this.f7324C = new byte[16];
        reset();
    }
    ...
    private void copyIn(MD2Digest mD2Digest) {
        byte[] bArr = mD2Digest.f7326X;
        System.arraycopy(bArr, 0, this.f7326X, 0, bArr.length);
        this.xOff = mD2Digest.xOff;
        byte[] bArr2 = mD2Digest.f7325M;
        System.arraycopy(bArr2, 0, this.f7325M, 0, bArr2.length);
        this.mOff = mD2Digest.mOff;
        byte[] bArr3 = mD2Digest.f7324C;
        System.arraycopy(bArr3, 0, this.f7324C, 0, bArr3.length);
    ...
    public int doFinal(byte[] bArr, int i) {
        int length = this.f7325M.length;
        int i2 = this.mOff;
        byte b = (byte) (length - i2);
        while (true) {
            byte[] bArr2 = this.f7325M;
            if (i2 >= bArr2.length) {
                processCheckSum(bArr2);
                processBlock(this.f7325M);
                processBlock(this.f7324C);
                System.arraycopy(this.f7326X, this.xOff, bArr, i, 16);
                reset();
                return 16;
            }
            bArr2[i2] = b;
    ...
    protected void processBlock(byte[] bArr) {
        ...
        for (int i3 = 0; i3 < 18; i3++) {
            for (int i4 = 0; i4 < 48; i4++) {
                byte[] bArr3 = this.f7326X;
                byte b = (byte) (f7323S[i2] ^ bArr3[i4]);
                bArr3[i4] = b;
                i2 = b & 255;
            }
            i2 = (i2 + i3) % 256;
    ...
    protected void processCheckSum(byte[] bArr) {
        byte b = this.f7324C[15];
        for (int i = 0; i < 16; i++) {
            byte[] bArr2 = this.f7324C;
            b = (byte) (f7323S[(b ^ bArr[i]) & 255] ^ bArr2[i]);
            bArr2[i] = b;
        }
    }
    ...
    public void update(byte[] bArr, int i, int i2) {
        ...
        while (i2 > 16) {
            System.arraycopy(bArr, i, this.f7325M, 0, 16);
            processCheckSum(this.f7325M);
            processBlock(this.f7325M);
            i2 -= 16;
            i += 16;
        }
