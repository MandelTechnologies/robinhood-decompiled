...
public class OCBBlockCipher implements AEADBlockCipher {
    ...
    protected static byte[] OCB_double(byte[] bArr) {
        ...
        bArr2[15] = (byte) ((EnumC7081g.SDK_ASSET_ILLUSTRATION_DEV_RAISE_INSTITUTION_VALUE >>> ((1 - iShiftLeft) << 3)) ^ bArr2[15]);
        return bArr2;
    }
    ...
    protected byte[] getLSub(int i) {
        while (i >= this.f7625L.size()) {
            Vector vector = this.f7625L;
            vector.addElement(OCB_double((byte[]) vector.lastElement()));
        }
        return (byte[]) this.f7625L.elementAt(i);
    }
