...
public class OpenSSHPrivateKeyUtil {
    ...
    public static byte[] encodePrivateKey(AsymmetricKeyParameter asymmetricKeyParameter) throws IOException {
        ...
        if (!(asymmetricKeyParameter instanceof RSAPrivateCrtKeyParameters) && !(asymmetricKeyParameter instanceof ECPrivateKeyParameters)) {
            ...
            sSHBuilder.writeString(ApiRenderablePlatforms.VERSIONS_NONE);
            sSHBuilder.writeString(ApiRenderablePlatforms.VERSIONS_NONE);
    ...
    public static AsymmetricKeyParameter parsePrivateKeyBlob(byte[] bArr) {
        AsymmetricKeyParameter eCPrivateKeyParameters = null;
        if (bArr[0] == 48) {
            ...
            if (aSN1Sequence.size() == 6) {
                ...
                eCPrivateKeyParameters = new ECPrivateKeyParameters(eCPrivateKey.getKey(), new ECNamedDomainParameters(aSN1ObjectIdentifier, ECNamedCurveTable.getByOID(aSN1ObjectIdentifier)));
            }
        } else {
            SSHBuffer sSHBuffer = new SSHBuffer(AUTH_MAGIC, bArr);
            if (!ApiRenderablePlatforms.VERSIONS_NONE.equals(sSHBuffer.readString())) {
            ...
            if ("ssh-ed25519".equals(string2)) {
                ...
                eCPrivateKeyParameters = new Ed25519PrivateKeyParameters(block, 0);
                ...
                sSHBuffer2.readBlock();
                eCPrivateKeyParameters = new ECPrivateKeyParameters(new BigInteger(1, sSHBuffer2.readBlock()), new ECNamedDomainParameters(byName, byOID));
            }
