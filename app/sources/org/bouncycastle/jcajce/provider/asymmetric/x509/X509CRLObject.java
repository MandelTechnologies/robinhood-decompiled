...
class X509CRLObject extends X509CRLImpl {
    ...
    private X509CRLInternal getInternalCRL() throws IOException {
        ...
        synchronized (this.cacheLock) {
            try {
                ...
                try {
                    encoded = this.f7756c.getEncoded(ASN1Encoding.DER);
                    x509CRLException = null;
                } catch (IOException e) {
                    encoded = null;
                    x509CRLException = new X509CRLException(e);
                }
                X509CRLInternal x509CRLInternal3 = new X509CRLInternal(this.bcHelper, this.f7756c, this.sigAlgName, this.sigAlgParams, this.isIndirect, encoded, x509CRLException);
                synchronized (this.cacheLock) {
                    try {
    ...
    public boolean equals(Object obj) throws IOException {
        ...
        if (obj instanceof X509CRLObject) {
            ...
            } else if ((this.internalCRLValue == null || x509CRLObject.internalCRLValue == null) && (signature = this.f7756c.getSignature()) != null && !signature.equals((ASN1Primitive) x509CRLObject.f7756c.getSignature())) {
                return false;
            }
