...
import org.bouncycastle.jcajce.p521io.OutputStreamFactory;
...
abstract class X509CertificateImpl extends X509Certificate implements BCX509Certificate {
    ...
    private void checkSignature(PublicKey publicKey, Signature signature, ASN1Encodable aSN1Encodable, byte[] bArr) throws NoSuchAlgorithmException, SignatureException, IOException, InvalidKeyException, CertificateException, InvalidAlgorithmParameterException {
        if (!isAlgIdEqual(this.f7757c.getSignatureAlgorithm(), this.f7757c.getTBSCertificate().getSignature())) {
        ...
        try {
            ...
            this.f7757c.getTBSCertificate().encodeTo(bufferedOutputStream, ASN1Encoding.DER);
    ...
    private void doVerify(PublicKey publicKey, SignatureCreator signatureCreator) throws NoSuchAlgorithmException, SignatureException, IOException, InvalidKeyException, CertificateException, NoSuchProviderException, InvalidAlgorithmParameterException {
        ...
        int i = 0;
        if (z && X509SignatureUtil.isCompositeAlgorithm(this.f7757c.getSignatureAlgorithm())) {
            List<PublicKey> publicKeys = ((CompositePublicKey) publicKey).getPublicKeys();
            ASN1Sequence aSN1Sequence = ASN1Sequence.getInstance(this.f7757c.getSignatureAlgorithm().getParameters());
            ASN1Sequence aSN1Sequence2 = ASN1Sequence.getInstance(DERBitString.getInstance((Object) this.f7757c.getSignature()).getBytes());
            boolean z2 = false;
        ...
        if (!X509SignatureUtil.isCompositeAlgorithm(this.f7757c.getSignatureAlgorithm())) {
            Signature signatureCreateSignature = signatureCreator.createSignature(X509SignatureUtil.getSignatureName(this.f7757c.getSignatureAlgorithm()));
            if (!z) {
                checkSignature(publicKey, signatureCreateSignature, this.f7757c.getSignatureAlgorithm().getParameters(), getSignature());
                return;
            }
            List<PublicKey> publicKeys2 = ((CompositePublicKey) publicKey).getPublicKeys();
            while (i != publicKeys2.size()) {
                try {
                    checkSignature(publicKeys2.get(i), signatureCreateSignature, this.f7757c.getSignatureAlgorithm().getParameters(), getSignature());
                    return;
        ...
        ASN1Sequence aSN1Sequence3 = ASN1Sequence.getInstance(this.f7757c.getSignatureAlgorithm().getParameters());
        ASN1Sequence aSN1Sequence4 = ASN1Sequence.getInstance(DERBitString.getInstance((Object) this.f7757c.getSignature()).getBytes());
        boolean z3 = false;
    ...
    public void checkValidity(Date date) throws CertificateNotYetValidException, CertificateExpiredException {
        if (date.getTime() > getNotAfter().getTime()) {
            throw new CertificateExpiredException("certificate expired on " + this.f7757c.getEndDate().getTime());
        }
        if (date.getTime() >= getNotBefore().getTime()) {
            return;
        }
        throw new CertificateNotYetValidException("certificate not valid till " + this.f7757c.getStartDate().getTime());
    }
    ...
    public Set getCriticalExtensionOIDs() {
        ...
        Extensions extensions = this.f7757c.getTBSCertificate().getExtensions();
        if (extensions == null) {
            return null;
        }
    ...
    public List getExtendedKeyUsage() throws CertificateParsingException {
        byte[] extensionOctets = getExtensionOctets(this.f7757c, "2.5.29.37");
        if (extensionOctets == null) {
            return null;
        }
        try {
    ...
    public byte[] getExtensionValue(String str) {
        ASN1OctetString extensionValue = getExtensionValue(this.f7757c, str);
        if (extensionValue == null) {
            return null;
        }
        try {
    ...
    public Collection getIssuerAlternativeNames() throws CertificateParsingException {
        return getAlternativeNames(this.f7757c, Extension.issuerAlternativeName.getId());
    }

    @Override // java.security.cert.X509Certificate
    public Principal getIssuerDN() {
        return new X509Principal(this.f7757c.getIssuer());
    }

    @Override // java.security.cert.X509Certificate
    public boolean[] getIssuerUniqueID() {
        ASN1BitString issuerUniqueId = this.f7757c.getTBSCertificate().getIssuerUniqueId();
    ...
    public X500Name getIssuerX500Name() {
        return this.f7757c.getIssuer();
    }

    @Override // java.security.cert.X509Certificate
    public X500Principal getIssuerX500Principal() {
        try {
            return new X500Principal(this.f7757c.getIssuer().getEncoded(ASN1Encoding.DER));
    ...
    public Date getNotAfter() {
        return this.f7757c.getEndDate().getDate();
    }

    @Override // java.security.cert.X509Certificate
    public Date getNotBefore() {
        return this.f7757c.getStartDate().getDate();
    }

    @Override // java.security.cert.Certificate
    public PublicKey getPublicKey() {
        try {
            return BouncyCastleProvider.getPublicKey(this.f7757c.getSubjectPublicKeyInfo());
        } catch (IOException unused) {
            return null;
        }
    }
    ...
    public BigInteger getSerialNumber() {
        return this.f7757c.getSerialNumber().getValue();
    }
    ...
    public String getSigAlgOID() {
        return this.f7757c.getSignatureAlgorithm().getAlgorithm().getId();
    }
    ...
    public byte[] getSignature() {
        return this.f7757c.getSignature().getOctets();
    }

    @Override // java.security.cert.X509Certificate
    public Collection getSubjectAlternativeNames() throws CertificateParsingException {
        return getAlternativeNames(this.f7757c, Extension.subjectAlternativeName.getId());
    }

    @Override // java.security.cert.X509Certificate
    public Principal getSubjectDN() {
        return new X509Principal(this.f7757c.getSubject());
    }

    @Override // java.security.cert.X509Certificate
    public boolean[] getSubjectUniqueID() {
        ASN1BitString subjectUniqueId = this.f7757c.getTBSCertificate().getSubjectUniqueId();
    ...
    public X500Name getSubjectX500Name() {
        return this.f7757c.getSubject();
    }

    @Override // java.security.cert.X509Certificate
    public X500Principal getSubjectX500Principal() {
        try {
            return new X500Principal(this.f7757c.getSubject().getEncoded(ASN1Encoding.DER));
        } catch (IOException unused) {
    ...
    public byte[] getTBSCertificate() throws CertificateEncodingException {
        try {
            return this.f7757c.getTBSCertificate().getEncoded(ASN1Encoding.DER);
        } catch (IOException e) {
            throw new CertificateEncodingException(e.toString());
        }
    }
    ...
    public TBSCertificate getTBSCertificateNative() {
        return this.f7757c.getTBSCertificate();
    }

    @Override // java.security.cert.X509Certificate
    public int getVersion() {
        return this.f7757c.getVersionNumber();
    }

    @Override // java.security.cert.X509Extension
    public boolean hasUnsupportedCriticalExtension() {
        Extensions extensions;
        if (getVersion() != 3 || (extensions = this.f7757c.getTBSCertificate().getExtensions()) == null) {
            return false;
        }
        Enumeration enumerationOids = extensions.oids();
        while (enumerationOids.hasMoreElements()) {
