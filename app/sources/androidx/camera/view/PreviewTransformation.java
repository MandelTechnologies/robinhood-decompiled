...
final class PreviewTransformation {
    ...
    void setTransformationInfo(SurfaceRequest.TransformationInfo transformationInfo, Size size, boolean z) {
        Logger.m45d("PreviewTransform", "Transformation info set: " + transformationInfo + PlaceholderUtils.XXShortPlaceholderText + size + PlaceholderUtils.XXShortPlaceholderText + z);
    ...
    void transformView(Size size, int i, View view) {
        if (size.getHeight() == 0 || size.getWidth() == 0) {
            Logger.m50w("PreviewTransform", "Transform not applied due to PreviewView size: " + size);
            return;
        }
        if (isTransformationInfoReady()) {
            if (view instanceof TextureView) {
                ...
                boolean z = false;
                boolean z2 = (!this.mHasCameraTransform || display == null || display.getRotation() == this.mTargetRotation) ? false : true;
                if (!this.mHasCameraTransform && getRemainingRotationDegrees() != 0) {
                    z = true;
                }
                if (z2 || z) {
                    Logger.m47e("PreviewTransform", "Custom rotation not supported with SurfaceView/PERFORMANCE mode.");
                }
            }
    ...
    private static void setMatrixRectToRect(Matrix matrix, RectF rectF, RectF rectF2, PreviewView.ScaleType scaleType) {
        ...
        switch (C09161.$SwitchMap$androidx$camera$view$PreviewView$ScaleType[scaleType.ordinal()]) {
            case 1:
            case 2:
            ...
            case 6:
                ...
                break;
            default:
                Logger.m47e("PreviewTransform", "Unexpected crop rect: " + scaleType);
