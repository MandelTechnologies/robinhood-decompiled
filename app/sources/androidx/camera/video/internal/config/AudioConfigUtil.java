...
public final class AudioConfigUtil {
    public static AudioMimeInfo resolveAudioMimeInfo(MediaSpec mediaSpec, VideoValidatedEncoderProfilesProxy videoValidatedEncoderProfilesProxy) {
        ...
        if (videoValidatedEncoderProfilesProxy == null || videoValidatedEncoderProfilesProxy.getDefaultAudioProfile() == null) {
            ...
            if (Objects.equals(mediaType, "audio/none")) {
                Logger.m45d("AudioConfigUtil", "EncoderProfiles contains undefined AUDIO mime type so cannot be used. May rely on fallback defaults to derive settings [chosen mime type: " + strOutputFormatToAudioMime + "(profile: " + iOutputFormatToAudioProfile + ")]");
            } else if (mediaSpec.getOutputFormat() == -1) {
                Logger.m45d("AudioConfigUtil", "MediaSpec contains OUTPUT_FORMAT_AUTO. Using EncoderProfiles to derive AUDIO settings [mime type: " + mediaType + "(profile: " + profile + ")]");
                ...
                Logger.m45d("AudioConfigUtil", "MediaSpec audio mime/profile matches EncoderProfiles. Using EncoderProfiles to derive AUDIO settings [mime type: " + mediaType + "(profile: " + iOutputFormatToAudioProfile + ")]");
                strOutputFormatToAudioMime = mediaType;
            } else {
                Logger.m45d("AudioConfigUtil", "MediaSpec audio mime or profile does not match EncoderProfiles, so EncoderProfiles settings cannot be used. May rely on fallback defaults to derive AUDIO settings [EncoderProfiles mime type: " + mediaType + "(profile: " + profile + "), chosen mime type: " + strOutputFormatToAudioMime + "(profile: " + iOutputFormatToAudioProfile + ")]");
            }
            defaultAudioProfile = null;
        }
    ...
    static int resolveAudioSource(AudioSpec audioSpec) {
        ...
        if (source == -1) {
            Logger.m45d("AudioConfigUtil", "Using default AUDIO source: 5");
            return 5;
        }
        Logger.m45d("AudioConfigUtil", "Using provided AUDIO source: " + source);
        return source;
    }
    ...
    static int resolveAudioSourceFormat(AudioSpec audioSpec) {
        ...
        if (sourceFormat == -1) {
            Logger.m45d("AudioConfigUtil", "Using default AUDIO source format: 2");
            return 2;
        }
        Logger.m45d("AudioConfigUtil", "Using provided AUDIO source format: " + sourceFormat);
        return sourceFormat;
    }
    ...
    static int selectSampleRateOrNearestSupported(Range<Integer> range, int i, int i2, final int i3) {
        ...
        while (true) {
            if (range.contains((Range<Integer>) Integer.valueOf(iIntValue))) {
                ...
                Logger.m45d("AudioConfigUtil", "Sample rate " + iIntValue + "Hz is not supported by audio source with channel count " + i + " and source format " + i2);
            } else {
                Logger.m45d("AudioConfigUtil", "Sample rate " + iIntValue + "Hz is not in target range " + range);
            }
            if (arrayList == null) {
                Logger.m45d("AudioConfigUtil", "Trying common sample rates in proximity order to target " + i3 + "Hz");
                ...
                Collections.sort(arrayList, new Comparator() { // from class: androidx.camera.video.internal.config.AudioConfigUtil$$ExternalSyntheticLambda0
                    ...
                    public final int compare(Object obj, Object obj2) {
                        return AudioConfigUtil.m4713$r8$lambda$Nnb9VBUPywxDlNTzwxMJAGgKF0(i3, (Integer) obj, (Integer) obj2);
                    }
                });
            }
            if (i4 < arrayList.size()) {
                ...
                Logger.m45d("AudioConfigUtil", "No sample rate found in target range or supported by audio source. Falling back to default sample rate of 44100Hz");
                return 44100;
            }
        }
    }

    /* renamed from: $r8$lambda$Nnb9VBUPywxDlNTzwxMJAGgKF-0, reason: not valid java name */
    public static /* synthetic */ int m4713$r8$lambda$Nnb9VBUPywxDlNTzwxMJAGgKF0(int i, Integer num, Integer num2) {
        float fSignum;
    ...
    static int scaleAndClampBitrate(int i, int i2, int i3, int i4, int i5, Range<Integer> range) {
        ...
        Logger.m45d("AudioConfigUtil", str);
        return iDoubleValue;
    }
}
