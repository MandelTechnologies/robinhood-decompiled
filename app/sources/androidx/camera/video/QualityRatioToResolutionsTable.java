...
class QualityRatioToResolutionsTable {
    ...
    static {
        ...
        map.put(Quality.f31SD, Range.create(Integer.valueOf(EnumC7081g.SDK_ASSET_ILLUSTRATION_PLAID_LOGO_NAVBAR_VALUE), 719));
    ...
    QualityRatioToResolutionsTable(List<Size> list, Map<Quality, Size> map) {
        for (Quality quality : sQualityRangeMap.keySet()) {
            this.mTable.put(QualityRatio.m82of(quality, -1), new ArrayList());
            Iterator<Integer> it = sAspectRatioMap.keySet().iterator();
            while (it.hasNext()) {
                this.mTable.put(QualityRatio.m82of(quality, it.next().intValue()), new ArrayList());
            }
        }
    ...
    private List<Size> getQualityRatioRow(Quality quality, int i) {
        return this.mTable.get(QualityRatio.m82of(quality, i));
    }
    ...
    static abstract class QualityRatio {
        ...
        /* renamed from: of */
        static QualityRatio m82of(Quality quality, int i) {
