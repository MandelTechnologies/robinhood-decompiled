...
class CameraStateMachine {
    ...
    public void updateState(CameraInternal.State state, CameraState.StateError stateError) {
        ...
        switch (C07801.$SwitchMap$androidx$camera$core$impl$CameraInternal$State[state.ordinal()]) {
            case 1:
        ...
        Logger.m45d("CameraStateMachine", "New public camera state " + cameraStateOnCameraPendingOpen + " from " + state + " and " + stateError);
        if (Objects.equals(this.mCameraStates.getValue(), cameraStateOnCameraPendingOpen)) {
            return;
        }
        Logger.m45d("CameraStateMachine", "Publishing new public camera state " + cameraStateOnCameraPendingOpen);
