...
class CameraSelectionOptimizer {
    ...
    private static String decideSkippedCameraIdByHeuristic(CameraManagerCompat cameraManagerCompat, Integer num, List<String> list) throws CameraAccessExceptionCompat {
        if (num != null && list.contains("0") && list.contains(Constants.SdidMigrationStatusCodes.ALREADY_SDID)) {
            if (num.intValue() == 1) {
            ...
            } else if (num.intValue() == 0 && ((Integer) cameraManagerCompat.getCameraCharacteristicsCompat(Constants.SdidMigrationStatusCodes.ALREADY_SDID).get(CameraCharacteristics.LENS_FACING)).intValue() == 0) {
                return "0";
            }
        }
        return null;
