...
public final class VectorizedFloatAnimationSpec<V extends AnimationVectors> implements VectorizedFiniteAnimationSpec<V> {
    ...
    public V getEndVelocity(V initialValue, V targetValue, V initialVelocity) {
        if (this.endVelocityVector == null) {
            this.endVelocityVector = (V) AnimationVectors6.newInstance(initialVelocity);
        }
    ...
    public long getDurationNanos(V initialValue, V targetValue, V initialVelocity) {
        ...
        long jMax = 0;
        for (int i = 0; i < size; i++) {
            jMax = Math.max(jMax, this.anims.get(i).getDurationNanos(initialValue.get$animation_core_release(i), targetValue.get$animation_core_release(i), initialVelocity.get$animation_core_release(i)));
        }
        return jMax;
    }
}
