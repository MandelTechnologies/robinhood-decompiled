...
import androidx.compose.p011ui.text.input.DeleteSurroundingTextInCodePointsCommand;
...
import androidx.compose.p011ui.text.input.SetComposingRegionCommand;
import androidx.compose.p011ui.text.input.SetComposingTextCommand;
...
public final class RecordingInputConnection implements InputConnection {
        ...
        if (z) {
            ...
            switch (id) {
                case R.id.selectAll:
                    ...
                    break;
                case R.id.cut:
                    sendSynthesizedKeyEvent(EnumC7081g.SDK_ASSET_ILLUSTRATION_DEBIT_CARD_OVERLAY_PIGGY_SOLID_VALUE);
                    break;
                case R.id.copy:
                    sendSynthesizedKeyEvent(EnumC7081g.SDK_ASSET_INSTITUTION_CIRCLE_SOLID_VALUE);
                    break;
                case R.id.paste:
                    sendSynthesizedKeyEvent(EnumC7081g.SDK_ASSET_ILLUSTRATION_SUCCESS_CIRCLE_SOLID_VALUE);
                    break;
            }
        }
        return z;
    ...
    public boolean performEditorAction(int editorAction) {
        int iM7746getDefaulteUduSuo;
        boolean z = this.isActive;
        if (!z) {
            return z;
        }
        if (editorAction == 0) {
            iM7746getDefaulteUduSuo = ImeAction.INSTANCE.m7746getDefaulteUduSuo();
        } else {
            switch (editorAction) {
                case 2:
                    iM7746getDefaulteUduSuo = ImeAction.INSTANCE.m7748getGoeUduSuo();
                    break;
                case 3:
                    iM7746getDefaulteUduSuo = ImeAction.INSTANCE.m7752getSearcheUduSuo();
                    break;
                case 4:
                    iM7746getDefaulteUduSuo = ImeAction.INSTANCE.m7753getSendeUduSuo();
                    break;
                case 5:
                    iM7746getDefaulteUduSuo = ImeAction.INSTANCE.m7749getNexteUduSuo();
                    break;
                case 6:
                    iM7746getDefaulteUduSuo = ImeAction.INSTANCE.m7747getDoneeUduSuo();
                    break;
                case 7:
                    iM7746getDefaulteUduSuo = ImeAction.INSTANCE.m7751getPreviouseUduSuo();
                    break;
                default:
        ...
        this.eventCallback.mo5472onImeActionKlQnJC8(iM7746getDefaulteUduSuo);
        return true;
    }
    ...
    public final void updateInputState(TextFieldValue state, InputMethodManager inputMethodManager) {
        if (this.isActive) {
            ...
            int iM7644getMinimpl = composition != null ? TextRange.m7644getMinimpl(composition.getPackedValue()) : -1;
            TextRange composition2 = state.getComposition();
            inputMethodManager.updateSelection(TextRange.m7644getMinimpl(state.getSelection()), TextRange.m7643getMaximpl(state.getSelection()), iM7644getMinimpl, composition2 != null ? TextRange.m7643getMaximpl(composition2.getPackedValue()) : -1);
        }
    }
    ...
    public CharSequence getTextBeforeCursor(int maxChars, int flags) {
        return TextFieldValue4.getTextBeforeSelection(this.textFieldValue, maxChars).toString();
    }

    @Override // android.view.inputmethod.InputConnection
    public CharSequence getTextAfterCursor(int maxChars, int flags) {
        return TextFieldValue4.getTextAfterSelection(this.textFieldValue, maxChars).toString();
    }

    @Override // android.view.inputmethod.InputConnection
    public CharSequence getSelectedText(int flags) {
        if (TextRange.m7640getCollapsedimpl(this.textFieldValue.getSelection())) {
            return null;
        }
        return TextFieldValue4.getSelectedText(this.textFieldValue).toString();
    }
    ...
    public int getCursorCapsMode(int reqModes) {
        return TextUtils.getCapsMode(this.textFieldValue.getText(), TextRange.m7644getMinimpl(this.textFieldValue.getSelection()), reqModes);
    }
}
