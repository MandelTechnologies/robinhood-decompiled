...
final class LazyGridPrefetchStrategy2 implements LazyGridPrefetchStrategy {
    ...
    public LazyGridPrefetchStrategy2(int i) {
        this.nestedPrefetchItemCount = i;
    }

    @Override // androidx.compose.foundation.lazy.grid.LazyGridPrefetchStrategy
    public void onScroll(LazyGridPrefetchStrategy3 lazyGridPrefetchStrategy3, float f, LazyGridLayoutInfo lazyGridLayoutInfo) {
        int row;
        int index;
        ...
        if (row != this.lineToPrefetch && row >= 0) {
            ...
            mutableVector2.addAll(mutableVector2.getSize(), (List<? extends LazyLayoutPrefetchState.PrefetchHandle>) lazyGridPrefetchStrategy3.scheduleLinePrefetch(row));
        }
        if (!z) {
            if (lazyGridLayoutInfo.getViewportStartOffset() - LazyGridSnapLayoutInfoProvider.offsetOnMainAxis((LazyGridItemInfo) CollectionsKt.first((List) lazyGridLayoutInfo.getVisibleItemsInfo()), lazyGridLayoutInfo.getOrientation()) < f) {
        ...
        if (((LazyGridSnapLayoutInfoProvider.offsetOnMainAxis(lazyGridItemInfo3, lazyGridLayoutInfo.getOrientation()) + LazyGridSnapLayoutInfoProvider.sizeOnMainAxis(lazyGridItemInfo3, lazyGridLayoutInfo.getOrientation())) + lazyGridLayoutInfo.getMainAxisItemSpacing()) - lazyGridLayoutInfo.getViewportEndOffset() < (-f)) {
    ...
    @Override // androidx.compose.foundation.lazy.grid.LazyGridPrefetchStrategy
    public void onVisibleItemsUpdated(LazyGridPrefetchStrategy3 lazyGridPrefetchStrategy3, LazyGridLayoutInfo lazyGridLayoutInfo) {
        int row;
