...
public final class FlowLayoutBuildingBlocks {
    ...
    /* compiled from: FlowLayoutBuildingBlocks.kt */
    @Metadata(m3635d1 = {"\u0000\u0012\n\u0002\u0018\u0002\n\u0002\u0010\u0000\n\u0000\n\u0002\u0010\u000b\n\u0002\b\u0004\b\u0007\u0018\u00002\u00020\u0001B\u0019\u0012\b\b\u0002\u0010\u0002\u001a\u00020\u0003\u0012\b\b\u0002\u0010\u0004\u001a\u00020\u0003¢\u0006\u0002\u0010\u0005R\u0011\u0010\u0004\u001a\u00020\u0003¢\u0006\b\n\u0000\u001a\u0004\b\u0004\u0010\u0006R\u0011\u0010\u0002\u001a\u00020\u0003¢\u0006\b\n\u0000\u001a\u0004\b\u0002\u0010\u0006¨\u0006\u0007"}, m3636d2 = {"Landroidx/compose/foundation/layout/FlowLayoutBuildingBlocks$WrapInfo;", "", "isLastItemInLine", "", "isLastItemInContainer", "(ZZ)V", "()Z", "foundation-layout_release"}, m3637k = 1, m3638mv = {1, 9, 0}, m3640xi = 48)
    ...
    public final WrapEllipsisInfo getWrapEllipsisInfo(WrapInfo wrapInfo, boolean hasNext, int lastContentLineIndex, int totalCrossAxisSize, int leftOverMainAxis, int nextIndexInLine) {
        ...
        wrapEllipsisInfoEllipsisInfo$foundation_layout_release.setPlaceEllipsisOnLastContentLine(lastContentLineIndex >= 0 && (nextIndexInLine == 0 || (leftOverMainAxis - IntIntPair.m4739getFirstimpl(wrapEllipsisInfoEllipsisInfo$foundation_layout_release.getEllipsisSize()) >= 0 && nextIndexInLine < this.maxItemsInMainAxis)));
        return wrapEllipsisInfoEllipsisInfo$foundation_layout_release;
    }

    /* renamed from: getWrapInfo-OpUlnko, reason: not valid java name */
    public final WrapInfo m5107getWrapInfoOpUlnko(boolean nextItemHasNext, int nextIndexInLine, long leftOver, IntIntPair nextSize, int lineIndex, int totalCrossAxisSize, int currentLineCrossAxisSize, boolean isWrappingRound, boolean isEllipsisWrap) {
        int i = totalCrossAxisSize + currentLineCrossAxisSize;
        if (nextSize == null) {
            return new WrapInfo(true, true);
        }
        if (this.overflow.getType() != FlowLayoutOverflow.OverflowType.Visible && (lineIndex >= this.maxLines || IntIntPair.m4740getSecondimpl(leftOver) - IntIntPair.m4740getSecondimpl(nextSize.getPackedValue()) < 0)) {
            return new WrapInfo(true, true);
        }
        if (nextIndexInLine != 0 && (nextIndexInLine >= this.maxItemsInMainAxis || IntIntPair.m4739getFirstimpl(leftOver) - IntIntPair.m4739getFirstimpl(nextSize.getPackedValue()) < 0)) {
            if (isWrappingRound) {
                return new WrapInfo(true, true);
            }
            return new WrapInfo(true, m5107getWrapInfoOpUlnko(nextItemHasNext, 0, IntIntPair.m4736constructorimpl(Constraints.m7975getMaxWidthimpl(this.constraints), (IntIntPair.m4740getSecondimpl(leftOver) - this.crossAxisSpacing) - currentLineCrossAxisSize), IntIntPair.m4735boximpl(IntIntPair.m4736constructorimpl(IntIntPair.m4739getFirstimpl(nextSize.getPackedValue()) - this.mainAxisSpacing, IntIntPair.m4740getSecondimpl(nextSize.getPackedValue()))), lineIndex + 1, i, 0, true, false).getIsLastItemInContainer());
        }
        int iMax = totalCrossAxisSize + Math.max(currentLineCrossAxisSize, IntIntPair.m4740getSecondimpl(nextSize.getPackedValue()));
        IntIntPair intIntPairM5112ellipsisSizeF35zmw$foundation_layout_release = isEllipsisWrap ? null : this.overflow.m5112ellipsisSizeF35zmw$foundation_layout_release(nextItemHasNext, lineIndex, iMax);
        if (intIntPairM5112ellipsisSizeF35zmw$foundation_layout_release != null) {
            intIntPairM5112ellipsisSizeF35zmw$foundation_layout_release.getPackedValue();
            if (nextIndexInLine + 1 >= this.maxItemsInMainAxis || ((IntIntPair.m4739getFirstimpl(leftOver) - IntIntPair.m4739getFirstimpl(nextSize.getPackedValue())) - this.mainAxisSpacing) - IntIntPair.m4739getFirstimpl(intIntPairM5112ellipsisSizeF35zmw$foundation_layout_release.getPackedValue()) < 0) {
                if (isEllipsisWrap) {
                    return new WrapInfo(true, true);
                }
                WrapInfo wrapInfoM5107getWrapInfoOpUlnko = m5107getWrapInfoOpUlnko(false, 0, IntIntPair.m4736constructorimpl(Constraints.m7975getMaxWidthimpl(this.constraints), (IntIntPair.m4740getSecondimpl(leftOver) - this.crossAxisSpacing) - Math.max(currentLineCrossAxisSize, IntIntPair.m4740getSecondimpl(nextSize.getPackedValue()))), intIntPairM5112ellipsisSizeF35zmw$foundation_layout_release, lineIndex + 1, iMax, 0, true, true);
                return new WrapInfo(wrapInfoM5107getWrapInfoOpUlnko.getIsLastItemInContainer(), wrapInfoM5107getWrapInfoOpUlnko.getIsLastItemInContainer());
            }
        }
