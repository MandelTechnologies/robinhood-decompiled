...
public abstract class DelegatingNode extends Modifier.Node {
    ...
    protected final <T extends DelegatableNode> T delegate(T delegatableNode) {
        ...
        if (node.getIsAttached()) {
            InlineClassHelper4.throwIllegalStateException("Cannot delegate to an already attached node");
        }
        node.setAsDelegateTo$ui_release(getNode());
        int kindSet = getKindSet();
        int iCalculateNodeKindSetFromIncludingDelegates = NodeKind3.calculateNodeKindSetFromIncludingDelegates(node);
        ...
        if (getIsAttached()) {
            if ((iCalculateNodeKindSetFromIncludingDelegates & NodeKind.m7381constructorimpl(2)) != 0 && (kindSet & NodeKind.m7381constructorimpl(2)) == 0) {
                NodeChain nodes = DelegatableNode2.requireLayoutNode(this).getNodes();
            ...
            NodeKind3.autoInvalidateInsertedNode(node);
        }
        return delegatableNode;
    }
        ...
        for (Modifier.Node child = this.delegate; child != null; child = child.getChild()) {
            if (child == instance) {
                if (child.getIsAttached()) {
                    NodeKind3.autoInvalidateRemovedNode(child);
                ...
                int kindSet = getKindSet();
                int iCalculateNodeKindSetFromIncludingDelegates = NodeKind3.calculateNodeKindSetFromIncludingDelegates(this);
                updateNodeKindSet(iCalculateNodeKindSetFromIncludingDelegates, true);
                if (getIsAttached() && (kindSet & NodeKind.m7381constructorimpl(2)) != 0 && (NodeKind.m7381constructorimpl(2) & iCalculateNodeKindSetFromIncludingDelegates) == 0) {
                    NodeChain nodes = DelegatableNode2.requireLayoutNode(this).getNodes();
                    getNode().updateCoordinator$ui_release(null);
                    nodes.syncCoordinators();
                    return;
    ...
    private final void validateDelegateKindSet(int delegateKindSet, Modifier.Node delegateNode) {
        int kindSet = getKindSet();
        if ((delegateKindSet & NodeKind.m7381constructorimpl(2)) == 0 || (NodeKind.m7381constructorimpl(2) & kindSet) == 0 || (this instanceof LayoutModifierNode)) {
            return;
        }
        InlineClassHelper4.throwIllegalStateException("Delegating to multiple LayoutModifierNodes without the delegating node implementing LayoutModifierNode itself is not allowed.\nDelegating Node: " + this + "\nDelegate Node: " + delegateNode);
    }
    ...
    private final void updateNodeKindSet(int newKindSet, boolean recalculateOwner) {
        ...
        if (kindSet != newKindSet) {
            if (DelegatableNode2.isDelegationRoot(this)) {
            ...
            if (getIsAttached()) {
                ...
                if (recalculateOwner && parent == node) {
                    newKindSet = NodeKind3.calculateNodeKindSetFromIncludingDelegates(node);
