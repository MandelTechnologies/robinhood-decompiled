...
import androidx.compose.p011ui.text.android.style.IndentationFixSpan_androidKt;
...
public final class TextLayout {
    ...
    public TextLayout(CharSequence charSequence, float f, TextPaint textPaint, int i, TextUtils.TruncateAt truncateAt, int i2, float f2, float f3, boolean z, boolean z2, int i3, int i4, int i5, int i6, int i7, int i8, int[] iArr, int[] iArr2, LayoutIntrinsics layoutIntrinsics) {
        ...
        try {
            ...
            if (boringMetrics != null && layoutIntrinsics.getMaxIntrinsicWidth() <= f && !z5) {
                ...
                textDirectionHeuristic = textDirectionHeuristic2;
                textPaint2 = textPaint;
                layoutCreate = StaticLayoutFactory.INSTANCE.create(charSequence, textPaint2, iCeil, 0, charSequence.length(), textDirectionHeuristic, alignment, i9, truncateAt, (int) Math.ceil(d), f2, f3, i8, z, z2, i4, i5, i6, i7, iArr, iArr2);
            }
            ...
            this.lineCount = iMin;
            int i10 = iMin - 1;
            this.didExceedMaxLines = (iMin >= i9 && (layoutCreate.getEllipsisCount(i10) > 0 || layoutCreate.getLineEnd(i10) != charSequence.length())) ? z3 : z4;
            ...
            this.topPadding = Math.max(VerticalPaddings.m7676getTopPaddingimpl(verticalPaddings), VerticalPaddings.m7676getTopPaddingimpl(lineHeightPaddings));
            this.bottomPadding = Math.max(VerticalPaddings.m7675getBottomPaddingimpl(verticalPaddings), VerticalPaddings.m7675getBottomPaddingimpl(lineHeightPaddings));
    ...
    public final void fillLineHorizontalBounds$ui_text_release(int lineIndex, float[] array2) {
        float secondaryDownstream;
        float secondaryUpstream;
        int lineStart = getLineStart(lineIndex);
        int lineEnd = getLineEnd(lineIndex);
        int i = 0;
        if (!(array2.length >= (lineEnd - lineStart) * 2)) {
            InlineClassHelper5.throwIllegalArgumentException("array.size - arrayStart must be greater or equal than (endOffset - startOffset) * 2");
        }
        ...
        while (lineStart < lineEnd) {
            ...
            array2[i] = secondaryDownstream;
            array2[i + 1] = secondaryUpstream;
            i += 2;
            lineStart++;
        }
    }

    public final void fillBoundingBoxes(int startOffset, int endOffset, float[] array2, int arrayStart) {
        float secondaryDownstream;
        float secondaryUpstream;
        int length = getText().length();
        if (!(startOffset >= 0)) {
            InlineClassHelper5.throwIllegalArgumentException("startOffset must be > 0");
        }
        if (!(startOffset < length)) {
            InlineClassHelper5.throwIllegalArgumentException("startOffset must be less than text length");
        }
        if (!(endOffset > startOffset)) {
            InlineClassHelper5.throwIllegalArgumentException("endOffset must be greater than startOffset");
        }
        if (!(endOffset <= length)) {
            InlineClassHelper5.throwIllegalArgumentException("endOffset must be smaller or equal to text length");
        }
        if (!(array2.length - arrayStart >= (endOffset - startOffset) * 4)) {
            InlineClassHelper5.throwIllegalArgumentException("array.size - arrayStart must be greater or equal than (endOffset - startOffset) * 4");
        }
        ...
        while (true) {
            ...
            for (int iMax = Math.max(startOffset, lineStart); iMax < iMin; iMax++) {
                ...
                array2[i2] = secondaryDownstream;
                array2[i2 + 1] = lineTop;
                array2[i2 + 2] = secondaryUpstream;
                array2[i2 + 3] = lineBottom;
                i2 += 4;
            }
            if (i == lineForOffset2) {
                return;
