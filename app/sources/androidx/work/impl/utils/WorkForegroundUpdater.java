            ...
            public void run() {
                try {
                    if (!settableFutureCreate.isCancelled()) {
                        ...
                        WorkSpec workSpec = WorkForegroundUpdater.this.mWorkSpecDao.getWorkSpec(string2);
                        if (workSpec == null || workSpec.state.isFinished()) {
                            throw new IllegalStateException("Calls to setForegroundAsync() must complete before a ListenableWorker signals completion of work by returning an instance of Result.");
                        }
                        WorkForegroundUpdater.this.mForegroundProcessor.startForeground(string2, foregroundInfo);
                        context.startService(SystemForegroundDispatcher.createNotifyIntent(context, WorkSpec3.generationalId(workSpec), foregroundInfo));
                    }
