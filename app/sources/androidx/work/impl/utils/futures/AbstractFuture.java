...
public abstract class AbstractFuture<V> implements ListenableFuture<V> {
    ...
    public final V get(long timeout, TimeUnit unit) throws ExecutionException, InterruptedException, TimeoutException {
        ...
        String string3 = unit.toString();
        Locale locale = Locale.ROOT;
        String lowerCase = string3.toLowerCase(locale);
        String str = "Waited " + timeout + PlaceholderUtils.XXShortPlaceholderText + unit.toString().toLowerCase(locale);
        if (nanos + 1000 < 0) {
    ...
    private static final class SafeAtomicHelper extends AtomicHelper {
        ...
        boolean casWaiters(AbstractFuture<?> future, Waiter expect, Waiter update) {
            return C2031xc40028dd.m363m(this.waitersUpdater, future, expect, update);
        }

        @Override // androidx.work.impl.utils.futures.AbstractFuture.AtomicHelper
        boolean casListeners(AbstractFuture<?> future, Listener expect, Listener update) {
            return C2031xc40028dd.m363m(this.listenersUpdater, future, expect, update);
        }

        @Override // androidx.work.impl.utils.futures.AbstractFuture.AtomicHelper
        boolean casValue(AbstractFuture<?> future, Object expect, Object update) {
            return C2031xc40028dd.m363m(this.valueUpdater, future, expect, update);
        }
    }
