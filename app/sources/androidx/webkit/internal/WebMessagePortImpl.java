...
public class WebMessagePortImpl extends WebMessagePortCompat {
    ...
    public void postMessage(WebMessageCompat webMessageCompat) {
        ApiFeature.C3018M c3018m = WebViewFeatureInternal.WEB_MESSAGE_PORT_POST_MESSAGE;
        if (c3018m.isSupportedByFramework() && webMessageCompat.getType() == 0) {
            ApiHelperForM.postMessage(getFrameworksImpl(), compatToFrameworkMessage(webMessageCompat));
        } else {
            if (c3018m.isSupportedByWebView() && WebMessageAdapter.isMessagePayloadTypeSupportedByWebView(webMessageCompat.getType())) {
    ...
    @Override // androidx.webkit.WebMessagePortCompat
    public void close() {
        ApiFeature.C3018M c3018m = WebViewFeatureInternal.WEB_MESSAGE_PORT_CLOSE;
