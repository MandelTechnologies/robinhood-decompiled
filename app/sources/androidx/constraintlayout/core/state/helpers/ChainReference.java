...
public class ChainReference extends HelperReference {
    ...
    public void addChainElement(Object obj, float f, float f2, float f3, float f4, float f5) {
        ...
        if (!Float.isNaN(f)) {
            this.mMapWeights.put(string2, Float.valueOf(f));
        }
        if (!Float.isNaN(f2)) {
            this.mMapPreMargin.put(string2, Float.valueOf(f2));
        }
        if (!Float.isNaN(f3)) {
            this.mMapPostMargin.put(string2, Float.valueOf(f3));
        }
        if (!Float.isNaN(f4)) {
            if (this.mMapPreGoneMargin == null) {
                this.mMapPreGoneMargin = new HashMap<>();
            }
            this.mMapPreGoneMargin.put(string2, Float.valueOf(f4));
        }
        if (Float.isNaN(f5)) {
            return;
        }
        ...
        this.mMapPostGoneMargin.put(string2, Float.valueOf(f5));
    }
