...
final class SubtitleTranscodingTrackOutput implements TrackOutput {
    ...
    public void format(Format format2) {
        ...
        Assertions.checkArgument(MimeTypes.getTrackType(format2.sampleMimeType) == 3);
        if (!format2.equals(this.currentFormat)) {
            this.currentFormat = format2;
            this.currentSubtitleParser = this.subtitleParserFactory.supportsFormat(format2) ? this.subtitleParserFactory.create(format2) : null;
        }
        if (this.currentSubtitleParser == null) {
            this.delegate.format(format2);
        } else {
            this.delegate.format(format2.buildUpon().setSampleMimeType("application/x-media3-cues").setCodecs(format2.sampleMimeType).setSubsampleOffsetUs(Long.MAX_VALUE).setCueReplacementBehavior(this.subtitleParserFactory.getCueReplacementBehavior(format2)).build());
        }
    }
    ...
    public void sampleMetadata(final long j, final int i, int i2, int i3, TrackOutput.CryptoData cryptoData) {
        ...
        this.currentSubtitleParser.parse(this.sampleData, i4, i2, SubtitleParser.OutputOptions.allCues(), new Consumer() { // from class: androidx.media3.extractor.text.SubtitleTranscodingTrackOutput$$ExternalSyntheticLambda0
            ...
            public final void accept(Object obj) {
                this.f$0.outputSample((CuesWithTiming) obj, j, i);
            }
        });
        int i5 = i4 + i2;
