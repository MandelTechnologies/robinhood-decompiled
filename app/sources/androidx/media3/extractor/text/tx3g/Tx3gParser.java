...
public final class Tx3gParser implements SubtitleParser {
    ...
    public void parse(byte[] bArr, int i, int i2, SubtitleParser.OutputOptions outputOptions, Consumer<CuesWithTiming> consumer) {
        ...
        if (subtitleText.isEmpty()) {
            consumer.accept(new CuesWithTiming(ImmutableList.m999of(), -9223372036854775807L, -9223372036854775807L));
            return;
        }
        ...
        consumer.accept(new CuesWithTiming(ImmutableList.m1000of(new Cue.Builder().setText(spannableStringBuilder).setLine(fConstrainValue, 0).setLineAnchor(0).build()), -9223372036854775807L, -9223372036854775807L));
    }
    ...
    private void applyStyleRecord(ParsableByteArray parsableByteArray, SpannableStringBuilder spannableStringBuilder) {
        ...
        if (unsignedShort2 > spannableStringBuilder.length()) {
            Log.m538w("Tx3gParser", "Truncating styl end (" + unsignedShort2 + ") to cueText.length() (" + spannableStringBuilder.length() + ").");
            unsignedShort2 = spannableStringBuilder.length();
        }
        int i2 = unsignedShort2;
        if (unsignedShort >= i2) {
            Log.m538w("Tx3gParser", "Ignoring styl with start (" + unsignedShort + ") >= end (" + i2 + ").");
            return;
        }
