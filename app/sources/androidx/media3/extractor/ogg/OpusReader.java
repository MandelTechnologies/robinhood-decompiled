...
final class OpusReader extends StreamReader {
    ...
    protected boolean readHeaders(ParsableByteArray parsableByteArray, long j, StreamReader.SetupData setupData) throws ParserException {
        if (peekPacketStartsWith(parsableByteArray, OPUS_ID_HEADER_SIGNATURE)) {
            ...
            if (setupData.f9744format != null) {
                return true;
            }
            setupData.f9744format = new Format.Builder().setSampleMimeType("audio/opus").setChannelCount(channelCount).setSampleRate(48000).setInitializationData(listBuildInitializationData).build();
            return true;
        }
        byte[] bArr = OPUS_COMMENT_HEADER_SIGNATURE;
        if (peekPacketStartsWith(parsableByteArray, bArr)) {
            Assertions.checkStateNotNull(setupData.f9744format);
            ...
            setupData.f9744format = setupData.f9744format.buildUpon().setMetadata(vorbisComments.copyWithAppendedEntriesFrom(setupData.f9744format.metadata)).build();
            return true;
        }
        Assertions.checkStateNotNull(setupData.f9744format);
        return false;
