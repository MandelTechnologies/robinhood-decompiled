...
public final class VorbisUtil {
    ...
    public static ImmutableList<byte[]> parseVorbisCsdFromEsdsInitializationData(byte[] bArr) {
        ...
        return ImmutableList.m1001of(bArr2, bArr3);
    }
    ...
    public static CommentHeader readVorbisCommentHeader(ParsableByteArray parsableByteArray, boolean z, boolean z2) throws ParserException {
        ...
        for (int i = 0; i < littleEndianUnsignedInt; i++) {
            String string3 = parsableByteArray.readString((int) parsableByteArray.readLittleEndianUnsignedInt());
            strArr[i] = string3;
            length2 = length2 + 4 + string3.length();
        }
        if (z2 && (parsableByteArray.readUnsignedByte() & 1) == 0) {
            throw ParserException.createForMalformedContainer("framing bit expected to be set", null);
        }
        return new CommentHeader(string2, strArr, length2 + 1);
    }
    ...
    public static Metadata parseVorbisComments(List<String> list) {
        ...
        for (int i = 0; i < list.size(); i++) {
            ...
            if (strArrSplitAtFirst.length != 2) {
                Log.m538w("VorbisUtil", "Failed to parse Vorbis comment: " + str);
                ...
                try {
                    ...
                    Log.m539w("VorbisUtil", "Failed to parse vorbis picture", e);
                }
            } else {
    ...
    private static void readMappings(int i, VorbisBitArray vorbisBitArray) throws ParserException {
        ...
        for (int i2 = 0; i2 < bits; i2++) {
            ...
            if (bits2 != 0) {
                Log.m535e("VorbisUtil", "mapping type other than 0 not supported: " + bits2);
            } else {
