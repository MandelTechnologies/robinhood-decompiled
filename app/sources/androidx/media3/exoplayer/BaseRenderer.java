...
public abstract class BaseRenderer implements Renderer, RendererCapabilities {
    ...
    protected final ExoPlaybackException createRendererException(Throwable th, Format format2, int i) {
        return createRendererException(th, format2, false, i);
    }

    protected final ExoPlaybackException createRendererException(Throwable th, Format format2, boolean z, int i) {
        int formatSupport;
        if (format2 == null || this.throwRendererExceptionIsExecuting) {
            ...
            try {
                formatSupport = RendererCapabilities.getFormatSupport(supportsFormat(format2));
            } catch (ExoPlaybackException unused) {
            } finally {
                this.throwRendererExceptionIsExecuting = false;
            }
        }
        return ExoPlaybackException.createForRenderer(th, getName(), getIndex(), format2, formatSupport, z, i);
    }
        ...
        if (data != -4) {
            if (data == -5) {
                ...
                if (format2.subsampleOffsetUs != Long.MAX_VALUE) {
                    formatHolder.f9709format = format2.buildUpon().setSubsampleOffsetUs(format2.subsampleOffsetUs + this.streamOffsetUs).build();
                }
            }
            return data;
        }
