...
public final class MediaMetricsListener implements AnalyticsListener, PlaybackSessionManager.Listener {
    ...
    public static MediaMetricsListener create(Context context) {
        MediaMetricsManager mediaMetricsManagerM606m = MediaMetricsListener$$ExternalSyntheticApiModelOutline5.m606m(context.getSystemService("media_metrics"));
        if (mediaMetricsManagerM606m == null) {
            return null;
        }
        return new MediaMetricsListener(context, mediaMetricsManagerM606m.createPlaybackSession());
    }
    ...
    public void onSessionActive(AnalyticsListener.EventTime eventTime, String str) {
        ...
        if (mediaPeriodId == null || !mediaPeriodId.isAd()) {
            ...
            this.metricsBuilder = MediaMetricsListener$$ExternalSyntheticApiModelOutline2.m573m().setPlayerName("AndroidXMedia3").setPlayerVersion("1.4.1");
    ...
    public void onVideoSizeChanged(AnalyticsListener.EventTime eventTime, VideoSize videoSize) {
        ...
        if (pendingFormatUpdate != null) {
            Format format2 = pendingFormatUpdate.f9711format;
            if (format2.height == -1) {
                this.pendingVideoFormat = new PendingFormatUpdate(format2.buildUpon().setWidth(videoSize.width).setHeight(videoSize.height).build(), pendingFormatUpdate.selectionReason, pendingFormatUpdate.sessionId);
            }
        }
    }
    ...
    private void maybeUpdateMetricsBuilderValues(Player player, AnalyticsListener.Events events) {
        ...
        if (events.contains(2) && this.metricsBuilder != null && (drmInitData = getDrmInitData(player.getCurrentTracks().getGroups())) != null) {
            MediaMetricsListener$$ExternalSyntheticApiModelOutline27.m581m(Util.castNonNull(this.metricsBuilder)).setDrmType(getDrmType(drmInitData));
        }
    ...
    private void maybeReportPlaybackError(long j) throws NumberFormatException {
        ...
        this.playbackSession.reportPlaybackErrorEvent(MediaMetricsListener$$ExternalSyntheticApiModelOutline3.m584m().setTimeSinceCreatedMillis(j - this.startTimeMs).setErrorCode(errorInfo.errorCode).setSubErrorCode(errorInfo.subErrorCode).setException(playbackException).build());
    ...
    private void maybeReportTrackChanges(Player player, AnalyticsListener.Events events, long j) {
        ...
        if (canReportPendingFormatUpdate(this.pendingVideoFormat)) {
            ...
            Format format2 = pendingFormatUpdate.f9711format;
            if (format2.height != -1) {
                maybeUpdateVideoFormat(j, format2, pendingFormatUpdate.selectionReason);
                this.pendingVideoFormat = null;
            }
        }
        if (canReportPendingFormatUpdate(this.pendingAudioFormat)) {
            PendingFormatUpdate pendingFormatUpdate2 = this.pendingAudioFormat;
            maybeUpdateAudioFormat(j, pendingFormatUpdate2.f9711format, pendingFormatUpdate2.selectionReason);
            this.pendingAudioFormat = null;
        }
        if (canReportPendingFormatUpdate(this.pendingTextFormat)) {
            PendingFormatUpdate pendingFormatUpdate3 = this.pendingTextFormat;
            maybeUpdateTextFormat(j, pendingFormatUpdate3.f9711format, pendingFormatUpdate3.selectionReason);
    ...
    private void maybeReportNetworkChange(long j) {
        ...
        if (networkType != this.currentNetworkType) {
            ...
            this.playbackSession.reportNetworkEvent(MediaMetricsListener$$ExternalSyntheticApiModelOutline1.m562m().setNetworkType(networkType).setTimeSinceCreatedMillis(j - this.startTimeMs).build());
        }
    }
    ...
    private void maybeReportPlaybackStateChange(Player player, AnalyticsListener.Events events, long j) {
        ...
        if (this.currentPlaybackState != iResolveNewPlaybackState) {
            ...
            this.reportedEventsForCurrentSession = true;
            this.playbackSession.reportPlaybackStateEvent(MediaMetricsListener$$ExternalSyntheticApiModelOutline4.m595m().setState(this.currentPlaybackState).setTimeSinceCreatedMillis(j - this.startTimeMs).build());
        }
    }
    ...
    private void maybeUpdateVideoFormat(long j, Format format2, int i) {
        if (Util.areEqual(this.currentVideoFormat, format2)) {
            return;
        }
        if (this.currentVideoFormat == null && i == 0) {
            i = 1;
        }
        this.currentVideoFormat = format2;
        reportTrackChangeEvent(1, j, format2, i);
    }

    private void maybeUpdateAudioFormat(long j, Format format2, int i) {
        ...
        reportTrackChangeEvent(0, j, format2, i);
    }

    private void maybeUpdateTextFormat(long j, Format format2, int i) {
        if (Util.areEqual(this.currentTextFormat, format2)) {
            return;
        }
        ...
        reportTrackChangeEvent(2, j, format2, i);
    }

    private void reportTrackChangeEvent(int i, long j, Format format2, int i2) {
        TrackChangeEvent.Builder timeSinceCreatedMillis = MediaMetricsListener$$ExternalSyntheticApiModelOutline0.m561m(i).setTimeSinceCreatedMillis(j - this.startTimeMs);
        if (format2 != null) {
    ...
    private static ErrorInfo getErrorInfo(PlaybackException playbackException, Context context, boolean z) throws NumberFormatException {
        ...
        if (playbackException instanceof ExoPlaybackException) {
            ...
            z2 = exoPlaybackException.f9708type == 1;
        ...
        if (z3 || (th instanceof UdpDataSource.UdpDataSourceException)) {
            ...
            if (z3 && ((HttpDataSource$HttpDataSourceException) th).f9706type == 1) {
    ...
    private static int getDrmType(DrmInitData drmInitData) {
        for (int i = 0; i < drmInitData.schemeDataCount; i++) {
            ...
            if (uuid.equals(C2456C.WIDEVINE_UUID)) {
                return 3;
            }
            if (uuid.equals(C2456C.PLAYREADY_UUID)) {
                return 2;
            }
            if (uuid.equals(C2456C.CLEARKEY_UUID)) {
                return 6;
            }
        }
        return 1;
