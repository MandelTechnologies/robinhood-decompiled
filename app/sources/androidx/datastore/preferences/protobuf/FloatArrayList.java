...
final class FloatArrayList extends AbstractProtobufList<Float> implements Internal.FloatList, RandomAccess, PrimitiveNonBoxingCollection {
    ...
    public boolean equals(Object obj) {
        ...
        float[] fArr = floatArrayList.f9695array;
        for (int i = 0; i < this.size; i++) {
            if (Float.floatToIntBits(this.f9695array[i]) != Float.floatToIntBits(fArr[i])) {
                return false;
            }
        }
        return true;
    ...
    public int hashCode() {
        ...
        for (int i = 0; i < this.size; i++) {
            iFloatToIntBits = (iFloatToIntBits * 31) + Float.floatToIntBits(this.f9695array[i]);
        }
    ...
    public Internal.ProtobufList<Float> mutableCopyWithCapacity2(int i) {
        ...
        return new FloatArrayList(Arrays.copyOf(this.f9695array, i), this.size);
    }
    ...
    public void addFloat(float f) {
        ...
        if (i == fArr.length) {
            ...
            this.f9695array = fArr2;
        }
        float[] fArr3 = this.f9695array;
        int i2 = this.size;
        this.size = i2 + 1;
        fArr3[i2] = f;
    }
    ...
    private void addFloat(int i, float f) {
        ...
        if (i2 < fArr.length) {
            ...
            System.arraycopy(fArr, 0, fArr2, 0, i);
            System.arraycopy(this.f9695array, i, fArr2, i + 1, this.size - i);
            this.f9695array = fArr2;
        }
        this.f9695array[i] = f;
        this.size++;
    ...
    public boolean addAll(Collection<? extends Float> collection) {
        ...
        float[] fArr = this.f9695array;
        if (i3 > fArr.length) {
            this.f9695array = Arrays.copyOf(fArr, i3);
        }
        System.arraycopy(floatArrayList.f9695array, 0, this.f9695array, this.size, floatArrayList.size);
        this.size = i3;
        ((AbstractList) this).modCount++;
        return true;
    }
    ...
    public boolean remove(Object obj) {
        ensureIsMutable();
        for (int i = 0; i < this.size; i++) {
            if (obj.equals(Float.valueOf(this.f9695array[i]))) {
                float[] fArr = this.f9695array;
