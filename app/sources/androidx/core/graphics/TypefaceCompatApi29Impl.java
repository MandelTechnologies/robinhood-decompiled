...
public class TypefaceCompatApi29Impl extends TypefaceCompatBaseImpl {
    ...
    private Font findBaseFont(FontFamily fontFamily, int i) {
        FontStyle fontStyle = new FontStyle((i & 1) != 0 ? Timeline.MIDDLE_META_OFFSET_ANIMATION_DURATION : 400, (i & 2) != 0 ? 1 : 0);
    ...
    private static FontFamily getFontFamily(CancellationSignal cancellationSignal, FontsContractCompat.FontInfo[] fontInfoArr, ContentResolver contentResolver) throws IOException {
        ...
        for (FontsContractCompat.FontInfo fontInfo : fontInfoArr) {
            try {
                parcelFileDescriptorOpenFileDescriptor = contentResolver.openFileDescriptor(fontInfo.getUri(), Constants.REVENUE_AMOUNT_KEY, cancellationSignal);
            } catch (IOException e) {
                Log.w("TypefaceCompatApi29Impl", "Font load failed", e);
            }
