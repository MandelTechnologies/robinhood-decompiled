...
public abstract class MediaControllerCompat$Callback implements IBinder.DeathRecipient {
    ...
    private static class MediaControllerCallbackApi21 extends MediaController.Callback {
        ...
        /* JADX WARN: Type inference failed for: r1v0, types: [android.support.v4.media.session.MediaControllerCompat$PlaybackInfo] */
        @Override // android.media.session.MediaController.Callback
        public void onAudioInfoChanged(MediaController.PlaybackInfo playbackInfo) {
            MediaControllerCompat$Callback mediaControllerCompat$Callback = this.mCallback.get();
            if (mediaControllerCompat$Callback != 0) {
                final int playbackType = playbackInfo.getPlaybackType();
                final AudioAttributesCompat audioAttributesCompatWrap = AudioAttributesCompat.wrap(playbackInfo.getAudioAttributes());
                final int volumeControl = playbackInfo.getVolumeControl();
                final int maxVolume = playbackInfo.getMaxVolume();
                final int currentVolume = playbackInfo.getCurrentVolume();
