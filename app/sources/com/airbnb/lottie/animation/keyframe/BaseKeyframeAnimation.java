...
public abstract class BaseKeyframeAnimation<K, A> {
    ...
    public void setProgress(float f) {
        if (C4754L.isTraceEnabled()) {
            C4754L.beginSection("BaseKeyframeAnimation#setProgress");
        }
        if (this.keyframesWrapper.isEmpty()) {
            if (C4754L.isTraceEnabled()) {
                C4754L.endSection("BaseKeyframeAnimation#setProgress");
                return;
            }
            return;
        }
    ...
    public void notifyListeners() {
        if (C4754L.isTraceEnabled()) {
            C4754L.beginSection("BaseKeyframeAnimation#notifyListeners");
        }
        for (int i = 0; i < this.listeners.size(); i++) {
            this.listeners.get(i).onValueChanged();
        }
        if (C4754L.isTraceEnabled()) {
            C4754L.endSection("BaseKeyframeAnimation#notifyListeners");
        }
    }

    protected Keyframe<K> getCurrentKeyframe() {
        if (C4754L.isTraceEnabled()) {
            C4754L.beginSection("BaseKeyframeAnimation#getCurrentKeyframe");
        }
        Keyframe<K> currentKeyframe = this.keyframesWrapper.getCurrentKeyframe();
        if (C4754L.isTraceEnabled()) {
            C4754L.endSection("BaseKeyframeAnimation#getCurrentKeyframe");
        }
