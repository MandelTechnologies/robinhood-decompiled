...
final class AdapterMethodsFactory implements JsonAdapter.Factory {
    ...
    public JsonAdapter<?> create(final Type type2, final Set<? extends Annotation> set, final Moshi moshi) {
        final AdapterMethod adapterMethod = get(this.toAdapters, type2, set);
        final AdapterMethod adapterMethod2 = get(this.fromAdapters, type2, set);
        ...
        if (adapterMethod == null || adapterMethod2 == null) {
            try {
                jsonAdapterNextAdapter = moshi.nextAdapter(this, type2, set);
            } catch (IllegalArgumentException e) {
                throw new IllegalArgumentException("No " + (adapterMethod == null ? "@ToJson" : "@FromJson") + " adapter for " + Util.typeAnnotatedWithAnnotations(type2, set), e);
            }
        }
    ...
    public static AdapterMethodsFactory get(Object obj) throws SecurityException {
        ...
        for (Class<?> superclass = obj.getClass(); superclass != Object.class; superclass = superclass.getSuperclass()) {
            for (Method method : superclass.getDeclaredMethods()) {
                if (method.isAnnotationPresent(ToJson.class)) {
                    ...
                    AdapterMethod adapterMethod = get(arrayList, adapter.f10548type, adapter.annotations);
                    if (adapterMethod != null) {
                ...
                if (method.isAnnotationPresent(FromJson.class)) {
                    ...
                    AdapterMethod adapterMethod2 = get(arrayList2, adapterMethodFromAdapter.f10548type, adapterMethodFromAdapter.annotations);
                    if (adapterMethod2 != null) {
    ...
    static AdapterMethod toAdapter(Object obj, Method method) throws SecurityException {
    ...
    static AdapterMethod fromAdapter(Object obj, Method method) throws SecurityException {
        method.setAccessible(true);
        final Type genericReturnType = method.getGenericReturnType();
        final Set<? extends Annotation> setJsonAnnotations = Util.jsonAnnotations(method);
        final Type[] genericParameterTypes = method.getGenericParameterTypes();
    ...
    private static AdapterMethod get(List<AdapterMethod> list, Type type2, Set<? extends Annotation> set) {
        int size = list.size();
        for (int i = 0; i < size; i++) {
            AdapterMethod adapterMethod = list.get(i);
            if (Types.equals(adapterMethod.f10548type, type2) && adapterMethod.annotations.equals(set)) {
    ...
    static abstract class AdapterMethod {
        ...
        final Type f10548type;

        AdapterMethod(Type type2, Set<? extends Annotation> set, Object obj, Method method, int i, int i2, boolean z) {
            this.f10548type = Util.canonicalize(type2);
        ...
        public void bind(Moshi moshi, JsonAdapter.Factory factory) {
            ...
            if (this.jsonAdapters.length > 0) {
                ...
                for (int i = this.adaptersOffset; i < length; i++) {
                    ...
                    if (Types.equals(this.f10548type, type2) && this.annotations.equals(setJsonAnnotations)) {
                        jsonAdapterAdapter = moshi.nextAdapter(factory, type2, setJsonAnnotations);
                    } else {
                        jsonAdapterAdapter = moshi.adapter(type2, setJsonAnnotations);
                    }
