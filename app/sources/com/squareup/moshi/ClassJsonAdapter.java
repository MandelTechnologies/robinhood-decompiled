...
final class ClassJsonAdapter<T> extends JsonAdapter<T> {
    public static final JsonAdapter.Factory FACTORY = new JsonAdapter.Factory() { // from class: com.squareup.moshi.ClassJsonAdapter.1
        ...
        public JsonAdapter<?> create(Type type2, Set<? extends Annotation> set, Moshi moshi) throws IllegalAccessException, NoSuchFieldException, NoSuchMethodException, SecurityException, ClassNotFoundException, IllegalArgumentException {
            ...
            while (type2 != Object.class) {
                createFieldBindings(moshi, type2, treeMap);
                type2 = Types.getGenericSuperclass(type2);
            }
            return new ClassJsonAdapter(classFactory, treeMap).nullSafe();
        }

        private void throwIfIsCollectionClass(Type type2, Class<?> cls) {
            Class<?> rawType = Types.getRawType(type2);
        ...
        private void createFieldBindings(Moshi moshi, Type type2, Map<String, FieldBinding<?>> map) throws SecurityException {
            Json json;
            Class<?> rawType = Types.getRawType(type2);
            ...
            for (Field field : rawType.getDeclaredFields()) {
                if (includeField(zIsPlatformType, field.getModifiers()) && ((json = (Json) field.getAnnotation(Json.class)) == null || !json.ignore())) {
                    Type typeResolve = Util.resolve(type2, rawType, field.getGenericType());
    ...
    ClassJsonAdapter(ClassFactory<T> classFactory, Map<String, FieldBinding<?>> map) {
        ...
        this.options = JsonReader.Options.m3152of((String[]) map.keySet().toArray(new String[map.size()]));
    }

    @Override // com.squareup.moshi.JsonAdapter
    public T fromJson(JsonReader jsonReader) throws IOException, IllegalArgumentException {
        try {
    ...
    @Override // com.squareup.moshi.JsonAdapter
    public void toJson(JsonWriter jsonWriter, T t) throws IOException, IllegalArgumentException {
        try {
