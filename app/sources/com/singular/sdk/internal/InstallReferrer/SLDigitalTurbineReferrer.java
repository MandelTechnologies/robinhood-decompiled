...
public class SLDigitalTurbineReferrer implements SLInstallReferrerService {
    private static final SingularLog logger = SingularLog.getLogger(SLDigitalTurbineReferrer.class.getSimpleName());
    ...
    private String readContentProvider(Context context, String str, ContentProviderClient contentProviderClient) {
        ...
        try {
            ...
            logger.debug("Read Content Provider Payload is " + string2);
            cursorQuery.close();
            return string2;
        } catch (Throwable th) {
    ...
    private String readContentProviderIntentAction(Context context, String str) {
        ...
        try {
            if (Build.VERSION.SDK_INT >= 33) {
                listQueryIntentContentProviders = context.getPackageManager().queryIntentContentProviders(new Intent(Constants.DT_CONTENT_PROVIDER_INTENT_ACTION_NAME), PackageManager.ResolveInfoFlags.of(131072L));
            } else {
    ...
    private String readContentProviderWithClient(Context context, String str) {
        try {
            ...
            String contentProvider = readContentProvider(context, formatURIString(str, "trackers"), contentProviderClientAcquireUnstableContentProviderClient);
