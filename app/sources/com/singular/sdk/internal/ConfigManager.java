...
public class ConfigManager implements ApiConfig.ApiConfigCompletionHandler {
    ...
    private static final SingularLog logger = SingularLog.getLogger(ConfigManager.class.getSimpleName());
    ...
    private void setConfigEnqueued(long j) {
        try {
            SharedPreferences.Editor editorEdit = SingularInstance.getInstance().getContext().getSharedPreferences(Constants.PREF_CONFIG_MANAGER, 0).edit();
            editorEdit.putLong(Constants.CONFIG_MANAGER_CONFIG_REQUEST_LAST_ENQUEUED, j);
            editorEdit.commit();
        } catch (Throwable th) {
    ...
    private void saveConfig(SLRemoteConfiguration sLRemoteConfiguration, SingularInstance singularInstance) {
        try {
            SharedPreferences.Editor editorEdit = singularInstance.getContext().getSharedPreferences(Constants.PREF_CONFIG_MANAGER, 0).edit();
    ...
    private SLRemoteConfiguration loadConfig() {
        try {
            String string2 = SingularInstance.getInstance().getContext().getSharedPreferences(Constants.PREF_CONFIG_MANAGER, 0).getString(Constants.CONFIG_MANAGER_CONFIG_KEY, null);
            ...
            return new SLRemoteConfiguration(new JSONObject(string2));
        } catch (Throwable th) {
