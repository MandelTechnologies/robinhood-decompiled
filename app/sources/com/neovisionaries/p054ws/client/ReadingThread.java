...
class ReadingThread extends WebSocketThread {
    ...
    */
    private WebSocketFrame readFrame() {
        ...
        try {
            ...
            try {
                ...
                e = e;
                if (e instanceof NoMoreFrameException) {
                    this.mNotWaitForCloseFrame = true;
                    if (!this.mWebSocket.isMissingCloseFrameAllowed()) {
                        callOnError(e);
                        callOnFrameError(e, frame);
                    }
                }
                this.mWebSocket.sendFrame(createCloseFrame(e));
                return null;
    ...
    private WebSocketFrame createCloseFrame(WebSocketException webSocketException) {
        int i = 1002;
        switch (C57511.$SwitchMap$com$neovisionaries$ws$client$WebSocketError[webSocketException.getError().ordinal()]) {
            case 1:
            case 2:
            case 3:
            case 6:
    ...
    private boolean handleContinuationFrame(WebSocketFrame webSocketFrame) throws IOException {
    ...
    private byte[] getMessage(List<WebSocketFrame> list) throws IOException {
    ...
    private byte[] concatenatePayloads(List<WebSocketFrame> list) throws IOException {
        try {
            ...
            callOnError(webSocketException2);
            callOnMessageError(webSocketException2, list);
            this.mWebSocket.sendFrame(WebSocketFrame.createCloseFrame(1009, webSocketException2.getMessage()));
            return null;
        }
    }
    ...
    private class CloseTask extends TimerTask {
        ...
        /* synthetic */ CloseTask(ReadingThread readingThread, C57511 c57511) {
            this();
        }
