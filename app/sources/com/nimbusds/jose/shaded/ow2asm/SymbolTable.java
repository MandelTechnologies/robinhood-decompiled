...
final class SymbolTable {
    ...
    */
    ...
    int addUninitializedType(String str, int i) {
        int iHash = hash(EnumC7081g.SDK_ASSET_ILLUSTRATION_FORM_VALUE, str, i);
        for (Entry entry = get(iHash); entry != null; entry = entry.next) {
            if (entry.tag == 129 && entry.hashCode == iHash && entry.data == i && entry.value.equals(str)) {
                return entry.index;
            }
        }
        return addTypeInternal(new Entry(this.typeCount, EnumC7081g.SDK_ASSET_ILLUSTRATION_FORM_VALUE, str, i, iHash));
    }
    ...
    int addMergedType(int i, int i2) {
        ...
        long j3 = j | (j2 << 32);
        int iHash = hash(EnumC7081g.SDK_ASSET_ILLUSTRATION_CONSUMER_VALUE, i + i2);
        for (Entry entry = get(iHash); entry != null; entry = entry.next) {
        ...
        put(new Entry(this.typeCount, EnumC7081g.SDK_ASSET_ILLUSTRATION_CONSUMER_VALUE, j3, iHash)).info = iAddType;
        return iAddType;
    }
