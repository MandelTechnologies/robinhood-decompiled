...
public class ConcatKDF implements JCAAware<JCAContext> {
    ...
    public SecretKey deriveKey(SecretKey secretKey, int i, byte[] bArr) throws JOSEException, IOException {
        ...
        if (byteArray.length == iByteLength) {
            return new SecretKeySpec(byteArray, BiometricChangeManager.ALGORITHM);
        }
        return new SecretKeySpec(ByteUtils.subArray(byteArray, 0, iByteLength), BiometricChangeManager.ALGORITHM);
    }
