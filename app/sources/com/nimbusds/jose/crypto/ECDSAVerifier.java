...
public class ECDSAVerifier extends ECDSAProvider implements JWSVerifier {
    ...
    public boolean verify(JWSHeader jWSHeader, byte[] bArr, Base64URL base64URL) throws SignatureException, JOSEException, InvalidKeyException {
        ...
        byte[] bArrDecode = base64URL.decode();
        if (ECDSA.getSignatureByteArrayLength(jWSHeader.getAlgorithm()) != bArrDecode.length) {
            return false;
        }
        try {
            byte[] bArrTranscodeSignatureToDER = ECDSA.transcodeSignatureToDER(bArrDecode);
            ...
            try {
                ...
                return signerAndVerifier.verify(bArrTranscodeSignatureToDER);
