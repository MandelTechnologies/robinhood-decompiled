...
public final /* data */ class RequestReceiver extends TransactionRowState4 {
    ...
    public static /* synthetic */ RequestReceiver copy$default(RequestReceiver requestReceiver, Request request, int i, Object obj) {
        if ((i & 1) != 0) {
            request = requestReceiver.transaction;
        }
        return requestReceiver.copy(request);
    }
    ...
    public final RequestReceiver copy(Request transaction) {
        Intrinsics.checkNotNullParameter(transaction, "transaction");
        return new RequestReceiver(transaction);
    }
    ...
    public boolean equals(Object other) {
        ...
        return (other instanceof RequestReceiver) && Intrinsics.areEqual(this.transaction, ((RequestReceiver) other).transaction);
    }
    ...
    public String toString() {
        return "RequestReceiver(transaction=" + this.transaction + ")";
    }
    ...
    public RequestReceiver(Request transaction) {
        int i;
        super(null);
        Intrinsics.checkNotNullParameter(transaction, "transaction");
        TransactionRowState4.Button button = null;
        this.transaction = transaction;
        StringResource.Companion companion = StringResource.INSTANCE;
        this.primaryText = companion.invoke(C21284R.string.matcha_pending_request_receiver_label, Money.format$default(getTransaction().getAmount(), null, false, null, false, 0, null, false, null, false, false, 1023, null), getTransaction().getTransactor().getDisplayName());
        Boolean canRemind = getTransaction().getCanRemind();
        if (canRemind != null) {
            boolean zBooleanValue = canRemind.booleanValue();
            TransactionRowState4.Action.RemindRequest remindRequest = TransactionRowState4.Action.RemindRequest.INSTANCE;
            if (zBooleanValue) {
                i = C21284R.string.matcha_pending_transaction_action_remind;
            } else {
                i = C21284R.string.matcha_pending_transaction_action_reminded;
            }
            button = new TransactionRowState4.Button(remindRequest, i, null, zBooleanValue);
        }
        this.primaryButton = button;
        this.secondaryButton = new TransactionRowState4.Button(TransactionRowState4.Action.CancelRequest.INSTANCE, C11048R.string.general_label_cancel, companion.invoke(C21284R.string.matcha_transaction_cancel_request_success, getTransaction().getTransactor().getDisplayName()), false, 8, null);
    }
