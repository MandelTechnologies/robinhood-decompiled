...
public final class zzx {
    public static int zza(int i, int i2, String str) {
        ...
        if (i < 0) {
            strZza = zzy.zza("%s (%s) must not be negative", ApiOptionSimulatedReturnsConfigurationV3Request.ApiOptionUnderlying.TYPE_INDEX, Integer.valueOf(i));
        } else {
            if (i2 < 0) {
                throw new IllegalArgumentException("negative size: " + i2);
            }
            strZza = zzy.zza("%s (%s) must be less than size (%s)", ApiOptionSimulatedReturnsConfigurationV3Request.ApiOptionUnderlying.TYPE_INDEX, Integer.valueOf(i), Integer.valueOf(i2));
        }
