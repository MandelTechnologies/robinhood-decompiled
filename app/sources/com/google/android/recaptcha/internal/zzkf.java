...
final class zzkf {
    ...
    zzkf(String str, char[] cArr) {
        byte[] bArr = new byte[128];
        Arrays.fill(bArr, (byte) -1);
        for (int i = 0; i < cArr.length; i++) {
            char c = cArr[i];
            boolean z = true;
            zzjh.zzc(c < 128, "Non-ASCII character: %s", c);
            if (bArr[c] != -1) {
                z = false;
            }
            zzjh.zzc(z, "Duplicate character: %s", c);
            bArr[c] = (byte) i;
        }
    ...
    public final boolean equals(Object obj) {
        if (obj instanceof zzkf) {
            ...
            boolean z = zzkfVar.zzi;
            if (Arrays.equals(this.zzf, zzkfVar.zzf)) {
                return true;
            }
        }
        return false;
    ...
    final int zzb(char c) throws zzkh {
        if (c > 127) {
    ...
    private zzkf(String str, char[] cArr, byte[] bArr, boolean z) {
        ...
        try {
            ...
            int iZzb = zzkl.zzb(length, RoundingMode.UNNECESSARY);
            this.zzb = iZzb;
