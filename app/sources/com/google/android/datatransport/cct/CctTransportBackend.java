...
final class CctTransportBackend implements TransportBackend {
    ...
    private static int getPackageVersionCode(Context context) {
        try {
            ...
            Logging.m794e("CctTransportBackend", "Unable to find version code for package", e);
            return -1;
        }
    }
    ...
    public EventInternal decorate(EventInternal eventInternal) {
        ...
        return eventInternal.toBuilder().addMetadata("sdk-version", Build.VERSION.SDK_INT).addMetadata("model", Build.MODEL).addMetadata("hardware", Build.HARDWARE).addMetadata("device", Build.DEVICE).addMetadata(SingularAttributionManager.PRODUCT_PARAM_KEY, Build.PRODUCT).addMetadata("os-uild", Build.ID).addMetadata("manufacturer", Build.MANUFACTURER).addMetadata("fingerprint", Build.FINGERPRINT).addMetadata("tz-offset", getTzOffset()).addMetadata("net-type", getNetTypeValue(activeNetworkInfo)).addMetadata("mobile-subtype", getNetSubtypeValue(activeNetworkInfo)).addMetadata(PlaceTypes.COUNTRY, Locale.getDefault().getCountry()).addMetadata("locale", Locale.getDefault().getLanguage()).addMetadata("mcc_mnc", getMccMncOrEmpty(this.applicationContext)).addMetadata("application_build", Integer.toString(getPackageVersionCode(this.applicationContext))).build();
    }
    ...
    private BatchedLogRequest getRequestBody(BackendRequest backendRequest) {
        ...
        for (Map.Entry entry : map.entrySet()) {
            ...
            for (EventInternal eventInternal3 : (List) entry.getValue()) {
                ...
                Encoding encoding2 = encodedPayload.getEncoding();
                if (encoding2.equals(Encoding.m790of("proto"))) {
                    builderProtoBuilder = LogEvent.protoBuilder(encodedPayload.getBytes());
                } else if (encoding2.equals(Encoding.m790of("json"))) {
                    builderProtoBuilder = LogEvent.jsonBuilder(new String(encodedPayload.getBytes(), Charset.forName("UTF-8")));
                } else {
                    Logging.m796w("CctTransportBackend", "Received event of unsupported encoding %s. Skipping...", encoding2);
                }
    ...
    public HttpResponse doSend(HttpRequest httpRequest) throws IOException {
        Logging.m795i("CctTransportBackend", "Making request to: %s", httpRequest.url);
        ...
        httpURLConnection.setRequestProperty(Headers.USER_AGENT, String.format("datatransport/%s android/", "3.3.0"));
        httpURLConnection.setRequestProperty("Content-Encoding", ContentEncoding.GZIP);
        ...
        try {
            ...
            try {
                ...
                try {
                    ...
                    Logging.m795i("CctTransportBackend", "Status Code: %d", Integer.valueOf(responseCode));
                    Logging.m792d("CctTransportBackend", "Content-Type: %s", httpURLConnection.getHeaderField("Content-Type"));
                    Logging.m792d("CctTransportBackend", "Content-Encoding: %s", httpURLConnection.getHeaderField("Content-Encoding"));
                    if (responseCode == 302 || responseCode == 301 || responseCode == 307) {
                        return new HttpResponse(responseCode, new URL(httpURLConnection.getHeaderField(Headers.LOCATION)), 0L);
                    }
                    if (responseCode != 200) {
            ...
            e = e;
            Logging.m794e("CctTransportBackend", "Couldn't encode request, returning with 400", e);
            return new HttpResponse(400, null, 0L);
        } catch (ConnectException e2) {
            e = e2;
            Logging.m794e("CctTransportBackend", "Couldn't open connection, returning with 500", e);
    ...
    private static InputStream maybeUnGzip(InputStream inputStream, String str) throws IOException {
        return ContentEncoding.GZIP.equals(str) ? new GZIPInputStream(inputStream) : inputStream;
    }
        ...
        try {
            HttpResponse httpResponse = (HttpResponse) Retries.retry(5, new HttpRequest(urlOrThrow, requestBody, aPIKey), new Function() { // from class: com.google.android.datatransport.cct.CctTransportBackend$$ExternalSyntheticLambda0
                ...
                public final Object shouldRetry(Object obj, Object obj2) {
                    return CctTransportBackend.m9214$r8$lambda$ZhERf3WZxxKL1vqOg2FJv71EaQ((CctTransportBackend.HttpRequest) obj, (CctTransportBackend.HttpResponse) obj2);
                }
            });
            int i = httpResponse.code;
            if (i == 200) {
                return BackendResponse.m791ok(httpResponse.nextRequestMillis);
            }
            if (i < 500 && i != 404) {
                if (i == 400) {
            ...
            Logging.m794e("CctTransportBackend", "Could not make request to the backend", e);
            return BackendResponse.transientError();
        }
    }

    /* renamed from: $r8$lambda$ZhERf3WZ-xxKL1vqOg2FJv71EaQ, reason: not valid java name */
    public static /* synthetic */ HttpRequest m9214$r8$lambda$ZhERf3WZxxKL1vqOg2FJv71EaQ(HttpRequest httpRequest, HttpResponse httpResponse) {
        URL url = httpResponse.redirectUrl;
        if (url == null) {
            return null;
        }
        Logging.m792d("CctTransportBackend", "Following redirect to: %s", url);
