...
final class TransportFactoryImpl implements TransportFactory {
    ...
    public <T> Transport<T> getTransport(String str, Class<T> cls, Transformer<T, byte[]> transformer) {
        return getTransport(str, cls, Encoding.m790of("proto"), transformer);
    }

    @Override // com.google.android.datatransport.TransportFactory
    public <T> Transport<T> getTransport(String str, Class<T> cls, Encoding encoding2, Transformer<T, byte[]> transformer) {
        if (!this.supportedPayloadEncodings.contains(encoding2)) {
            throw new IllegalArgumentException(String.format("%s is not supported byt this factory. Supported encodings are: %s.", encoding2, this.supportedPayloadEncodings));
        }
