...
class CrashlyticsController {
    ...
    synchronized void handleUncaughtException(final SettingsProvider settingsProvider, final Thread thread, final Throwable th, final boolean z) throws Throwable {
        try {
            try {
                Logger.getLogger().m1023d("Handling uncaught exception \"" + th + "\" from thread " + thread.getName());
                ...
                try {
                    Utils.awaitEvenIfOnMainThread(this.backgroundWorker.submitTask(new Callable<Task<Void>>() { // from class: com.google.firebase.crashlytics.internal.common.CrashlyticsController.2
                        ...
                        public Task<Void> call() throws Exception {
                            ...
                            Logger.getLogger().m1025e("Tried to write a fatal exception while no session was open.");
                            return Tasks.forResult(null);
                        }
                    }));
                } catch (TimeoutException unused) {
                    Logger.getLogger().m1025e("Cannot send reports. Timed out while fetching settings.");
                } catch (Exception e) {
                    Logger.getLogger().m1026e("Error handling uncaught exception", e);
                }
            } catch (Throwable th2) {
                th = th2;
                throw th;
    ...
    private Task<Boolean> waitForReportAction() {
        if (this.dataCollectionArbiter.isAutomaticDataCollectionEnabled()) {
            Logger.getLogger().m1023d("Automatic data collection is enabled. Allowing upload.");
            this.unsentReportsAvailable.trySetResult(Boolean.FALSE);
            return Tasks.forResult(Boolean.TRUE);
        }
        Logger.getLogger().m1023d("Automatic data collection is disabled.");
        Logger.getLogger().m1029v("Notifying that unsent reports are available.");
        ...
        Logger.getLogger().m1023d("Waiting for send/deleteUnsentReports to be called.");
    ...
    boolean didCrashOnPreviousExecution() {
        ...
        Logger.getLogger().m1029v("Found previous crash marker.");
    ...
    Task<Void> submitAllReports(Task<Settings> task) {
        if (!this.reportingCoordinator.hasReportsToSend()) {
            Logger.getLogger().m1029v("No crash reports are available to be sent.");
            this.unsentReportsAvailable.trySetResult(Boolean.FALSE);
            return Tasks.forResult(null);
        }
        Logger.getLogger().m1029v("Crash reports are available to be sent.");
        return waitForReportAction().onSuccessTask(new C53824(task));
    }

    /* renamed from: com.google.firebase.crashlytics.internal.common.CrashlyticsController$4 */
    /* loaded from: classes27.dex */
    class C53824 implements SuccessContinuation<Boolean, Void> {
            ...
            return CrashlyticsController.this.backgroundWorker.submitTask(new Callable<Task<Void>>() { // from class: com.google.firebase.crashlytics.internal.common.CrashlyticsController.4.1
                ...
                public Task<Void> call() throws Exception {
                    if (!bool.booleanValue()) {
                        Logger.getLogger().m1029v("Deleting cached crash reports...");
                        CrashlyticsController.deleteFiles(CrashlyticsController.this.listAppExceptionMarkerFiles());
                        CrashlyticsController.this.reportingCoordinator.removeAllReports();
                        CrashlyticsController.this.unsentReportsHandled.trySetResult(null);
                        return Tasks.forResult(null);
                    }
                    Logger.getLogger().m1023d("Sending cached crash reports...");
                    CrashlyticsController.this.dataCollectionArbiter.grantDataCollectionPermission(bool.booleanValue());
                    final Executor executor = CrashlyticsController.this.backgroundWorker.getExecutor();
                    return C53824.this.val$settingsDataTask.onSuccessTask(executor, new SuccessContinuation<Settings, Void>() { // from class: com.google.firebase.crashlytics.internal.common.CrashlyticsController.4.1.1
                        @Override // com.google.android.gms.tasks.SuccessContinuation
                        public Task<Void> then(Settings settings) throws Exception {
                            if (settings != null) {
    ...
    void setCustomKey(String str, String str2) {
        try {
            ...
            Logger.getLogger().m1025e("Attempting to set custom attribute with null key, ignoring.");
        }
    }
    ...
    boolean finalizeSessions(SettingsProvider settingsProvider) {
        ...
        Logger.getLogger().m1029v("Finalizing previously open sessions.");
        try {
            doCloseSessions(true, settingsProvider);
            Logger.getLogger().m1029v("Closed all previously open sessions.");
            return true;
        } catch (Exception e) {
            Logger.getLogger().m1026e("Unable to finalize previously open sessions.", e);
            return false;
        }
    }
    ...
    public void doOpenSession(String str, Boolean bool) {
        ...
        Logger.getLogger().m1023d("Opening a new session with ID " + str);
    ...
    private void doCloseSessions(boolean z, SettingsProvider settingsProvider) throws IOException {
        ...
        if (arrayList.size() <= z) {
            Logger.getLogger().m1029v("No open sessions to be closed.");
            return;
        }
        ...
        if (settingsProvider.getSettingsSync().featureFlagData.collectAnrs) {
            ...
            Logger.getLogger().m1029v("ANR feature disabled.");
        }
    ...
    void saveVersionControlInfo() {
        try {
            ...
            if (versionControlInfo != null) {
                ...
                Logger.getLogger().m1027i("Saved version control info");
            }
        } catch (IOException e) {
            Logger.getLogger().m1032w("Unable to save version control info", e);
        }
    }
    ...
    String getVersionControlInfo() throws IOException {
        ...
        Logger.getLogger().m1023d("Read version control info");
    ...
    private InputStream getResourceAsStream(String str) {
        ...
        if (classLoader == null) {
            Logger.getLogger().m1031w("Couldn't get Class Loader");
            return null;
        }
        ...
        Logger.getLogger().m1027i("No version control information found");
        return null;
    }
    ...
    private void finalizePreviousNativeSession(String str) throws IOException {
        Logger.getLogger().m1029v("Finalizing native report for session " + str);
        ...
        if (nativeCoreAbsent(str, minidumpFile, applicationExitInto)) {
            Logger.getLogger().m1031w("No native core present");
            return;
        }
        ...
        Logger.getLogger().m1023d("CrashlyticsController#finalizePreviousNativeSession");
    ...
    private static boolean nativeCoreAbsent(String str, File file, CrashlyticsReport.ApplicationExitInfo applicationExitInfo) {
        if (file == null || !file.exists()) {
            Logger.getLogger().m1031w("No minidump data found for session " + str);
        }
        if (applicationExitInfo == null) {
            Logger.getLogger().m1027i("No Tombstones data found for session " + str);
        }
    ...
    private Task<Void> logAnalyticsAppExceptionEvent(final long j) {
        ...
        Logger.getLogger().m1023d("Logging app exception event to Firebase Analytics");
    ...
    private void writeApplicationExitInfoEventIfRelevant(String str) {
        ...
        if (i >= 30) {
            ...
            if (historicalProcessExitReasons.size() != 0) {
                ...
                Logger.getLogger().m1029v("No ApplicationExitInfo available. Session: " + str);
                return;
            }
        }
        Logger.getLogger().m1029v("ANR feature enabled, but device is API " + i);
    }
}
