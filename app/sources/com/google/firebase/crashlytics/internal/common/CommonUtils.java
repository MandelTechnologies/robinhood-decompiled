...
public class CommonUtils {
    ...
    static final class Architecture {
        ...
        static Architecture getValue() {
            ...
            if (TextUtils.isEmpty(str)) {
                Logger.getLogger().m1029v("Architecture#getValue()::Build.CPU_ABI returned null or empty");
                return UNKNOWN;
            }
    ...
    private static String hash(byte[] bArr, String str) throws NoSuchAlgorithmException {
        try {
            ...
            Logger.getLogger().m1026e("Could not create hashing algorithm: " + str + ", returning empty string.", e);
            return "";
        }
    }
    ...
    public static boolean isEmulator() {
        if (Build.PRODUCT.contains(Constants.RequestParamsKeys.SDK_VERSION_KEY)) {
            return true;
        }
    ...
    public static List<BuildIdInfo> getBuildIdInfo(Context context) throws Resources.NotFoundException {
        ArrayList arrayList = new ArrayList();
        int resourcesIdentifier = getResourcesIdentifier(context, "com.google.firebase.crashlytics.build_ids_lib", ResourceTypes.ARRAY);
        int resourcesIdentifier2 = getResourcesIdentifier(context, "com.google.firebase.crashlytics.build_ids_arch", ResourceTypes.ARRAY);
        int resourcesIdentifier3 = getResourcesIdentifier(context, "com.google.firebase.crashlytics.build_ids_build_id", ResourceTypes.ARRAY);
        if (resourcesIdentifier == 0 || resourcesIdentifier2 == 0 || resourcesIdentifier3 == 0) {
            Logger.getLogger().m1023d(String.format("Could not find resources: %d %d %d", Integer.valueOf(resourcesIdentifier), Integer.valueOf(resourcesIdentifier2), Integer.valueOf(resourcesIdentifier3)));
            return arrayList;
        }
        ...
        if (stringArray.length != stringArray3.length || stringArray2.length != stringArray3.length) {
            Logger.getLogger().m1023d(String.format("Lengths did not match: %d %d %d", Integer.valueOf(stringArray.length), Integer.valueOf(stringArray2.length), Integer.valueOf(stringArray3.length)));
            return arrayList;
        }
