...
class MetaDataStore {
    ...
    public void writeUserData(String str, String str2) throws Throwable {
        ...
        try {
            ...
            Logger.getLogger().m1032w("Error serializing user metadata.", e);
    ...
    public String readUserId(String str) throws Throwable {
        ...
        if (!userDataFileForSession.exists() || userDataFileForSession.length() == 0) {
            Logger.getLogger().m1023d("No userId set for session " + str);
        ...
        try {
            ...
            try {
                try {
                    ...
                    Logger.getLogger().m1023d("Loaded userId " + strJsonToUserId + " for session " + str);
                    CommonUtils.closeOrLog(fileInputStream, "Failed to close user metadata file.");
                    return strJsonToUserId;
                } catch (Exception e) {
                    e = e;
                    Logger.getLogger().m1032w("Error deserializing user metadata.", e);
                    safeDeleteCorruptFile(userDataFileForSession);
                    CommonUtils.closeOrLog(fileInputStream, "Failed to close user metadata file.");
                    return null;
                }
    ...
    public void writeKeyData(String str, Map<String, String> map, boolean z) throws Throwable {
        ...
        try {
            ...
            e = e2;
            bufferedWriter2 = bufferedWriter;
            Logger.getLogger().m1032w("Error serializing key/value metadata.", e);
    ...
    public void writeRolloutState(String str, List<RolloutAssignment> list) throws Throwable {
        ...
        try {
            ...
            e = e2;
            bufferedWriter2 = bufferedWriter;
            Logger.getLogger().m1032w("Error serializing rollouts state.", e);
            safeDeleteCorruptFile(rolloutsStateForSession);
    ...
    private static List<RolloutAssignment> jsonToRolloutsState(String str) throws JSONException {
        ...
        for (int i = 0; i < jSONArray.length(); i++) {
            ...
            try {
                arrayList.add(RolloutAssignment.create(string2));
            } catch (Exception e) {
                Logger.getLogger().m1032w("Failed de-serializing rollouts state. " + string2, e);
            }
        }
        return arrayList;
    }
    ...
    private static String rolloutsStateToJson(List<RolloutAssignment> list) {
        ...
        for (int i = 0; i < list.size(); i++) {
            try {
                ...
                Logger.getLogger().m1032w("Exception parsing rollout assignment!", e);
            }
        }
    ...
    private static void safeDeleteCorruptFile(File file) {
        if (file.exists() && file.delete()) {
            Logger.getLogger().m1027i("Deleted corrupt file: " + file.getAbsolutePath());
        }
    }
}
