...
class DefaultSettingsSpiCall implements SettingsSpiCall {
    ...
    protected HttpGetRequest createHttpGetRequest(Map<String, String> map) {
        return this.requestFactory.buildHttpGetRequest(this.url, map).header(Headers.USER_AGENT, "Crashlytics Android SDK/" + CrashlyticsCore.getVersion()).header("X-CRASHLYTICS-DEVELOPER-TOKEN", "470fa2b4ae81cd56ecbcda9735803434cec591fa");
    }
    ...
    public JSONObject invoke(SettingsRequest settingsRequest, boolean z) {
        ...
        try {
            ...
            this.logger.m1023d("Requesting settings from " + this.url);
            this.logger.m1029v("Settings query params were: " + queryParamsFor);
            return handleResponse(httpGetRequestApplyHeadersTo.execute());
        } catch (IOException e) {
            this.logger.m1026e("Settings request failed.", e);
            return null;
        }
    }
    ...
    JSONObject handleResponse(HttpResponse httpResponse) {
        ...
        this.logger.m1029v("Settings response code was: " + iCode);
        if (requestWasSuccessful(iCode)) {
            return getJsonObjectFrom(httpResponse.body());
        }
        this.logger.m1025e("Settings request failed; (status: " + iCode + ") from " + this.url);
        return null;
    }
    ...
    private JSONObject getJsonObjectFrom(String str) {
        try {
            ...
            this.logger.m1032w("Failed to parse settings JSON from " + this.url, e);
            this.logger.m1031w("Settings response " + str);
            return null;
        }
    }
