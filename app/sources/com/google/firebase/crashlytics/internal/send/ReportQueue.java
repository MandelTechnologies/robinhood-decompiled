...
final class ReportQueue {
        ...
        synchronized (this.queue) {
            try {
                ...
                if (z) {
                    ...
                    if (isQueueAvailable()) {
                        Logger.getLogger().m1023d("Enqueueing report: " + crashlyticsReportWithSessionId.getSessionId());
                        Logger.getLogger().m1023d("Queue size: " + this.queue.size());
                        this.singleThreadExecutor.execute(new ReportRunnable(crashlyticsReportWithSessionId, taskCompletionSource));
                        Logger.getLogger().m1023d("Closing task for report: " + crashlyticsReportWithSessionId.getSessionId());
                        taskCompletionSource.trySetResult(crashlyticsReportWithSessionId);
                        return taskCompletionSource;
                    }
                    calcStep();
                    Logger.getLogger().m1023d("Dropping report due to queue being full: " + crashlyticsReportWithSessionId.getSessionId());
    ...
    public void sendReport(final CrashlyticsReportWithSessionId crashlyticsReportWithSessionId, final TaskCompletionSource<CrashlyticsReportWithSessionId> taskCompletionSource) {
        Logger.getLogger().m1023d("Sending report through Google DataTransport: " + crashlyticsReportWithSessionId.getSessionId());
    ...
    private final class ReportRunnable implements Runnable {
        ...
        public void run() throws InterruptedException {
            ...
            Logger.getLogger().m1023d("Delay for: " + String.format(Locale.US, "%.2f", Double.valueOf(dCalcDelay / 1000.0d)) + " s for report: " + this.reportWithSessionId.getSessionId());
