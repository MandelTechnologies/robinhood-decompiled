...
public class CrashlyticsReportPersistence {
    ...
    public void persistReport(CrashlyticsReport crashlyticsReport) {
        ...
        if (session == null) {
            Logger.getLogger().m1023d("Could not get session for report");
            return;
        }
        ...
        try {
            ...
            Logger.getLogger().m1024d("Could not persist report for session " + identifier, e);
        }
    }
    ...
    public void persistEvent(CrashlyticsReport.Session.Event event, String str, boolean z) {
        ...
        try {
            ...
            Logger.getLogger().m1032w("Could not persist event for session " + str, e);
        }
        trimEvents(str, i);
    }
    ...
    public void finalizeReports(String str, long j) {
        for (String str2 : capAndGetOpenSessions(str)) {
            Logger.getLogger().m1029v("Finalizing report for session " + str2);
    ...
    public void finalizeSessionWithNativeEvent(String str, CrashlyticsReport.FilesPayload filesPayload, CrashlyticsReport.ApplicationExitInfo applicationExitInfo) {
        ...
        Logger.getLogger().m1023d("Writing native session report for " + str + " to file: " + sessionFile);
    ...
    public List<CrashlyticsReportWithSessionId> loadFinalizedReports() {
        ...
        for (File file : allFinalizedReportFiles) {
            try {
                ...
                Logger.getLogger().m1032w("Could not load report file " + file + "; deleting", e);
                file.delete();
            }
        }
        return arrayList;
    ...
    private SortedSet<String> capAndGetOpenSessions(String str) {
        ...
        if (openSessionIds.size() > 8) {
            while (openSessionIds.size() > 8) {
                ...
                Logger.getLogger().m1023d("Removing session over cap: " + strLast);
    ...
    private void synthesizeReport(String str, long j) {
        ...
        if (sessionFiles.isEmpty()) {
            Logger.getLogger().m1029v("Session " + str + " has no events.");
            return;
        }
        ...
        loop0: while (true) {
            ...
            for (File file : sessionFiles) {
                try {
                    ...
                    Logger.getLogger().m1032w("Could not add event to report for " + file, e);
                }
        ...
        if (arrayList.isEmpty()) {
            Logger.getLogger().m1031w("Could not parse event files for session " + str);
            return;
        }
    ...
    private void synthesizeNativeReportFile(File file, CrashlyticsReport.FilesPayload filesPayload, String str, CrashlyticsReport.ApplicationExitInfo applicationExitInfo) {
        ...
        try {
            ...
            Logger.getLogger().m1032w("Could not synthesize final native report file for " + file, e);
        }
    }
    ...
    private void synthesizeReportFile(File file, List<CrashlyticsReport.Session.Event> list, long j, boolean z, String str, String str2) {
        ...
        try {
            ...
            Logger.getLogger().m1023d("appQualitySessionId: " + str2);
            if (z) {
            ...
            Logger.getLogger().m1032w("Could not synthesize final report file for " + file, e);
        }
    }
