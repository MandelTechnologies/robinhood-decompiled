...
final class SqlTimeTypeAdapter extends TypeAdapter<Time> {
    ...
    private final DateFormat f9847format;

    private SqlTimeTypeAdapter() {
        this.f9847format = new SimpleDateFormat("hh:mm:ss a");
    }
    ...
    public Time read2(JsonReader jsonReader) throws IOException {
        ...
        synchronized (this) {
            TimeZone timeZone = this.f9847format.getTimeZone();
            try {
                try {
                    time = new Time(this.f9847format.parse(strNextString).getTime());
                } catch (ParseException e) {
                    throw new JsonSyntaxException("Failed parsing '" + strNextString + "' as SQL Time; at path " + jsonReader.getPreviousPath(), e);
                }
            } finally {
                this.f9847format.setTimeZone(timeZone);
            }
        }
        return time;
    }
    ...
    public void write(JsonWriter jsonWriter, Time time) throws IOException {
        ...
        synchronized (this) {
            str = this.f9847format.format((Date) time);
        }
