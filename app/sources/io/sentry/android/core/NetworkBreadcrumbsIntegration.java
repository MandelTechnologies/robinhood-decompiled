...
public final class NetworkBreadcrumbsIntegration implements Integration, Closeable {
    ...
    static final class NetworkBreadcrumbsNetworkCallback extends ConnectivityManager.NetworkCallback {
        ...
        public void onCapabilitiesChanged(Network network, NetworkCapabilities networkCapabilities) {
            ...
            if (network.equals(this.currentNetwork) && (newConnectionDetails = getNewConnectionDetails(this.lastCapabilities, networkCapabilities, this.lastCapabilityNanos, (jNanoTimestamp = this.dateProvider.now().nanoTimestamp()))) != null) {
                ...
                breadcrumbCreateBreadcrumb.setData("network_type", newConnectionDetails.f10715type);
                int i = newConnectionDetails.signalStrength;
                if (i != 0) {
                    breadcrumbCreateBreadcrumb.setData("signal_strength", Integer.valueOf(i));
                }
                Hint hint = new Hint();
                hint.set("android:networkCapabilities", newConnectionDetails);
    ...
    static class NetworkBreadcrumbConnectionDetail {
        ...
        NetworkBreadcrumbConnectionDetail(NetworkCapabilities networkCapabilities, BuildInfoProvider buildInfoProvider, long j) {
            ...
            this.f10715type = connectionType == null ? "" : connectionType;
        ...
        boolean isSimilar(NetworkBreadcrumbConnectionDetail networkBreadcrumbConnectionDetail) {
            ...
            return this.isVpn == networkBreadcrumbConnectionDetail.isVpn && this.f10715type.equals(networkBreadcrumbConnectionDetail.f10715type) && (z || iAbs <= 5) && (z || (((double) iAbs2) > Math.max(1000.0d, ((double) Math.abs(this.downBandwidth)) * 0.1d) ? 1 : (((double) iAbs2) == Math.max(1000.0d, ((double) Math.abs(this.downBandwidth)) * 0.1d) ? 0 : -1)) <= 0) && (z || (((double) iAbs3) > Math.max(1000.0d, ((double) Math.abs(this.upBandwidth)) * 0.1d) ? 1 : (((double) iAbs3) == Math.max(1000.0d, ((double) Math.abs(this.upBandwidth)) * 0.1d) ? 0 : -1)) <= 0);
        }
    }
}
