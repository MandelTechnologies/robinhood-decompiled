...
public final class SentryExceptionFactory {
    ...
    Deque<SentryException> extractExceptionQueue(Throwable th) {
        ...
        while (th != null && hashSet.add(th)) {
            ...
            Mechanism mechanism = exceptionMechanism;
            if (mechanism != null && Boolean.FALSE.equals(mechanism.isHandled())) {
                z = true;
            }
            arrayDeque.addFirst(getSentryException(th2, mechanism, Long.valueOf(threadCurrentThread.getId()), this.sentryStackTraceFactory.getStackFrames(th2.getStackTrace(), z), zIsSnapshot));
            th = th2.getCause();
        }
        return arrayDeque;
    }
