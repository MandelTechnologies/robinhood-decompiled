...
public final class DefaultTransactionPerformanceCollector implements TransactionPerformanceCollector {
            ...
            try {
                ...
                this.timer.scheduleAtFixedRate(new TimerTask() { // from class: io.sentry.DefaultTransactionPerformanceCollector.2
                    ...
                    public void run() {
                        ...
                        if (jCurrentTimeMillis - DefaultTransactionPerformanceCollector.this.lastCollectionTimestamp < 10) {
                            return;
                        }
                        DefaultTransactionPerformanceCollector.this.lastCollectionTimestamp = jCurrentTimeMillis;
    ...
    public List<PerformanceCollectionData> stop(ITransaction iTransaction) {
        ...
        List<PerformanceCollectionData> listRemove = this.performanceDataMap.remove(iTransaction.getEventId().toString());
        Iterator<IPerformanceContinuousCollector> it = this.continuousCollectors.iterator();
        while (it.hasNext()) {
