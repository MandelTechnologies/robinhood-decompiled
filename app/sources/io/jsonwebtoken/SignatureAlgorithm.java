...
public enum SignatureAlgorithm {
    NONE(ApiRenderablePlatforms.VERSIONS_NONE, "No digital signature or MAC performed", "None", null, false, 0, 0),
    ...
    ES512("ES512", "ECDSA using P-521 and SHA-512", "ECDSA", "SHA512withECDSA", true, 512, HttpStatusCode.DOWNTIME_ERROR),
    ...
    private void assertValid(Key key, boolean z) throws InvalidKeyException {
        ...
        if (isHmac()) {
            if (!(key instanceof SecretKey)) {
                throw new InvalidKeyException(this.familyName + PlaceholderUtils.XXShortPlaceholderText + keyType(z) + " keys must be SecretKey instances.");
            }
        ...
        if (isEllipticCurve()) {
            if (!(key instanceof ECKey)) {
                throw new InvalidKeyException(this.familyName + PlaceholderUtils.XXShortPlaceholderText + keyType(z) + " keys must be ECKey instances.");
            }
        ...
        if (!(key instanceof RSAKey)) {
            throw new InvalidKeyException(this.familyName + PlaceholderUtils.XXShortPlaceholderText + keyType(z) + " keys must be RSAKey instances.");
        }
    ...
    public static SignatureAlgorithm forSigningKey(Key key) throws InvalidKeyException {
        ...
        if (key instanceof RSAKey) {
            ...
            if (iBitLength >= signatureAlgorithm4.minKeyLength) {
                signatureAlgorithm4.assertValidSigningKey(key);
                return signatureAlgorithm4;
            }
            throw new WeakKeyException("The specified RSA signing key is not strong enough to be used with JWT RSA signature algorithms.  The JWT specification requires RSA keys to be >= 2048 bits long.  The specified RSA key is " + iBitLength + " bits.  See https://tools.ietf.org/html/rfc7518#section-3.3 for more information.");
        }
        int iBitLength2 = ((ECKey) key).getParams().getOrder().bitLength();
        ...
        throw new WeakKeyException("The specified Elliptic Curve signing key is not strong enough to be used with JWT ECDSA signature algorithms.  The JWT specification requires ECDSA keys to be >= 256 bits long.  The specified ECDSA key is " + iBitLength2 + " bits.  See https://tools.ietf.org/html/rfc7518#section-3.4 for more information.");
    }
