...
public final class ObservableWindowBoundarySelector<T, B, V> extends AbstractObservableWithUpstream<T, Observable<T>> {
    ...
    static final class WindowBoundaryMainObserver<T, B, V> extends QueueDrainObserver<T, Object, Observable<T>> implements Disposable {
        ...
        /* renamed from: ws */
        final List<UnicastSubject<T>> f6620ws;
        ...
        public void onSubscribe(Disposable disposable) {
            if (DisposableHelper.validate(this.upstream, disposable)) {
                ...
                if (PreviewView$1$$ExternalSyntheticBackportWithForwarding0.m88m(this.boundary, null, operatorWindowBoundaryOpenObserver)) {
        ...
        public void onNext(T t) {
            if (fastEnter()) {
                Iterator<UnicastSubject<T>> it = this.f6620ws.iterator();
                while (it.hasNext()) {
                    it.next().onNext(t);
                }
                if (leave(-1) == 0) {
        ...
        void drainLoop() {
            ...
            List<UnicastSubject<T>> list = this.f6620ws;
            int iLeave = 1;
            while (true) {
                boolean z = this.done;
                ...
                if (!z2) {
                    if (objPoll instanceof WindowOperation) {
                        ...
                        UnicastSubject<T> unicastSubject = windowOperation.f6621w;
                        if (unicastSubject != null) {
                            if (list.remove(unicastSubject)) {
                                windowOperation.f6621w.onComplete();
                                if (this.windows.decrementAndGet() == 0) {
                                    disposeBoundary();
                                    return;
                                }
        ...
        void close(OperatorWindowBoundaryCloseObserver<T, V> operatorWindowBoundaryCloseObserver) {
            ...
            this.queue.offer(new WindowOperation(operatorWindowBoundaryCloseObserver.f6619w, null));
            if (enter()) {
                drainLoop();
            }
        }
    ...
    static final class WindowOperation<T, B> {
        ...
        /* renamed from: w */
        final UnicastSubject<T> f6621w;

        WindowOperation(UnicastSubject<T> unicastSubject, B b) {
            this.f6621w = unicastSubject;
            this.open = b;
        }
    }
    ...
    static final class OperatorWindowBoundaryCloseObserver<T, V> extends DisposableObserver<V> {
        ...
        /* renamed from: w */
        final UnicastSubject<T> f6619w;

        OperatorWindowBoundaryCloseObserver(WindowBoundaryMainObserver<T, ?, V> windowBoundaryMainObserver, UnicastSubject<T> unicastSubject) {
            this.parent = windowBoundaryMainObserver;
            this.f6619w = unicastSubject;
        }
