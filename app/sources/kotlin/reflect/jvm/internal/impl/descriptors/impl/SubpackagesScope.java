...
public class SubpackagesScope extends MemberScopeImpl {
    ...
    public Collection<DeclarationDescriptor> getContributedDescriptors(MemberScope3 kindFilter, Function1<? super Name, Boolean> nameFilter) {
        ...
        if (!kindFilter.acceptsKinds(MemberScope3.Companion.getPACKAGES_MASK())) {
            return CollectionsKt.emptyList();
        }
        if (this.fqName.isRoot() && kindFilter.getExcludes().contains(MemberScope2.TopLevelPackages.INSTANCE)) {
            return CollectionsKt.emptyList();
        }
        ...
        while (it.hasNext()) {
            ...
            if (nameFilter.invoke(nameShortName).booleanValue()) {
                collections.addIfNotNull(arrayList, getPackage(nameShortName));
            }
        }
        return arrayList;
    }
