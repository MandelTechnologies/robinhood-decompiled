...
public final class OperatorChecks extends AbstractModifierChecks {
    ...
    public static final String checks$lambda$2(FunctionDescriptor Checks) {
        ...
        boolean z = false;
        if (valueParameterDescriptor != null && !DescriptorUtils2.declaresOrInheritsDefaultValue(valueParameterDescriptor) && valueParameterDescriptor.getVarargElementType() == null) {
            z = true;
        }
        if (z) {
            return null;
    ...
    */
    public static final String checks$lambda$6(FunctionDescriptor Checks) {
        ...
        if (!checks$lambda$6$isAny(containingDeclaration)) {
            ...
            if (collection.isEmpty()) {
                z = !descriptorUtil.isTypedEqualsInValueClass(Checks);
            } else {
                ...
                if (!descriptorUtil.isTypedEqualsInValueClass(Checks)) {
                }
            }
        }
        if (z) {
        ...
        if (inlineClassesUtils.isValueClass(containingDeclaration3)) {
            ...
            KotlinType4 defaultType = ((ClassDescriptor) containingDeclaration4).getDefaultType();
            Intrinsics.checkNotNullExpressionValue(defaultType, "getDefaultType(...)");
            sb.append(" or define ''equals(other: " + descriptorRenderer.renderType(TypeUtils2.replaceArgumentsWithStarProjections(defaultType)) + "): Boolean''");
        }
        return sb.toString();
    }
    ...
    public static final String checks$lambda$8(FunctionDescriptor Checks) {
        ...
        if (dispatchReceiverParameter != null) {
            ...
            if (returnType != null) {
                ...
                zIsSubtypeOf = TypeUtils2.isSubtypeOf(returnType, type2);
            } else {
                zIsSubtypeOf = false;
            }
    ...
    private final boolean incDecCheckForExpectClass(FunctionDescriptor functionDescriptor, ReceiverParameterDescriptor receiverParameterDescriptor) {
        ...
        if (!classDescriptor.isExpect() || (classId = DescriptorUtils2.getClassId(classDescriptor)) == null) {
            return false;
        }
        ClassifierDescriptor classifierDescriptorFindClassifierAcrossModuleDependencies = findClassInModule.findClassifierAcrossModuleDependencies(DescriptorUtils2.getModule(classDescriptor), classId);
        ...
        return TypeUtils2.isSubtypeOf(returnType, typeAliasDescriptor.getExpandedType());
    }
}
