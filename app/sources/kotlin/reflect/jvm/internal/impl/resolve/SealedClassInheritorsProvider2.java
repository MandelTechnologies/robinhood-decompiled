...
/* renamed from: kotlin.reflect.jvm.internal.impl.resolve.CliSealedClassInheritorsProvider, reason: use source file name */
/* loaded from: classes14.dex */
public final class SealedClassInheritorsProvider2 extends SealedClassInheritorsProvider {
    public static final SealedClassInheritorsProvider2 INSTANCE = new SealedClassInheritorsProvider2();

    private SealedClassInheritorsProvider2() {
    }
    ...
    public Collection<ClassDescriptor> computeSealedSubclasses(ClassDescriptor sealedClass, boolean z) {
        ...
        if (!z) {
            ...
            Iterator<DeclarationDescriptor> itIterator2 = DescriptorUtils2.getParents(sealedClass).iterator2();
            while (true) {
    ...
    private static final void computeSealedSubclasses$collectSubclasses(ClassDescriptor classDescriptor, LinkedHashSet<ClassDescriptor> linkedHashSet, MemberScope memberScope, boolean z) {
        for (DeclarationDescriptor declarationDescriptor : ResolutionScope.DefaultImpls.getContributedDescriptors$default(memberScope, MemberScope3.CLASSIFIERS, null, 2, null)) {
            if (declarationDescriptor instanceof ClassDescriptor) {
                ClassDescriptor classDescriptor2 = (ClassDescriptor) declarationDescriptor;
                if (classDescriptor2.isExpect()) {
                    Name name = classDescriptor2.getName();
                    Intrinsics.checkNotNullExpressionValue(name, "getName(...)");
                    ClassifierDescriptor classifierDescriptorMo28706getContributedClassifier = memberScope.mo28706getContributedClassifier(name, LookupLocation3.WHEN_GET_ALL_DESCRIPTORS);
                    if (classifierDescriptorMo28706getContributedClassifier instanceof ClassDescriptor) {
                        classDescriptor2 = (ClassDescriptor) classifierDescriptorMo28706getContributedClassifier;
                    } else {
                        classDescriptor2 = classifierDescriptorMo28706getContributedClassifier instanceof TypeAliasDescriptor ? ((TypeAliasDescriptor) classifierDescriptorMo28706getContributedClassifier).getClassDescriptor() : null;
                    }
                }
