package rosetta.mainst;

import com.squareup.wire.EnumAdapter;
import com.squareup.wire.ProtoAdapter;
import com.squareup.wire.Syntax;
import com.squareup.wire.WireEnum;
import kotlin.Metadata;
import kotlin.enums.EnumEntries;
import kotlin.enums.EnumEntries2;
import kotlin.jvm.JvmField;
import kotlin.jvm.JvmStatic;
import kotlin.jvm.internal.DefaultConstructorMarker;
import kotlin.jvm.internal.Reflection;
import kotlin.reflect.KClass;

/* JADX WARN: Failed to restore enum class, 'enum' modifier and super class removed */
/* JADX WARN: Unknown enum class pattern. Please report as an issue! */
/* compiled from: InstrumentType.kt */
@Metadata(m3635d1 = {"\u0000\u0016\n\u0002\u0018\u0002\n\u0002\u0018\u0002\n\u0002\u0010\u0010\n\u0000\n\u0002\u0010\b\n\u0002\b\u0012\b\u0086\u0081\u0002\u0018\u0000 \u00152\u00020\u00012\b\u0012\u0004\u0012\u00020\u00000\u0002:\u0001\u0015B\u0011\b\u0002\u0012\u0006\u0010\u0003\u001a\u00020\u0004¢\u0006\u0004\b\u0005\u0010\u0006R\u0014\u0010\u0003\u001a\u00020\u0004X\u0096\u0004¢\u0006\b\n\u0000\u001a\u0004\b\u0007\u0010\bj\u0002\b\tj\u0002\b\nj\u0002\b\u000bj\u0002\b\fj\u0002\b\rj\u0002\b\u000ej\u0002\b\u000fj\u0002\b\u0010j\u0002\b\u0011j\u0002\b\u0012j\u0002\b\u0013j\u0002\b\u0014¨\u0006\u0016"}, m3636d2 = {"Lrosetta/mainst/InstrumentType;", "Lcom/squareup/wire/WireEnum;", "", "value", "", "<init>", "(Ljava/lang/String;II)V", "getValue", "()I", "INSTRUMENT_TYPE_UNSPECIFIED", "EQUITY", "OPTION", "CRYPTO", "FOREX", "CURRENCY", "FUTURES", "INDEX", "EVENT_CONTRACT", "CRYPTO_PERPETUAL", "INSTRUMENT_TYPE_FIAT", "OPTION_CHAIN", "Companion", "rosetta.mainst_externalRelease"}, m3637k = 1, m3638mv = {2, 1, 0}, m3640xi = 48)
/* loaded from: classes23.dex */
public final class InstrumentType implements WireEnum {
    private static final /* synthetic */ EnumEntries $ENTRIES;
    private static final /* synthetic */ InstrumentType[] $VALUES;

    @JvmField
    public static final ProtoAdapter<InstrumentType> ADAPTER;
    public static final InstrumentType CRYPTO;
    public static final InstrumentType CRYPTO_PERPETUAL;
    public static final InstrumentType CURRENCY;

    /* renamed from: Companion, reason: from kotlin metadata */
    public static final Companion INSTANCE;
    public static final InstrumentType EQUITY;
    public static final InstrumentType EVENT_CONTRACT;
    public static final InstrumentType FOREX;
    public static final InstrumentType FUTURES;
    public static final InstrumentType INDEX;
    public static final InstrumentType INSTRUMENT_TYPE_FIAT;
    public static final InstrumentType INSTRUMENT_TYPE_UNSPECIFIED;
    public static final InstrumentType OPTION;
    public static final InstrumentType OPTION_CHAIN;
    private final int value;

    private static final /* synthetic */ InstrumentType[] $values() {
        return new InstrumentType[]{INSTRUMENT_TYPE_UNSPECIFIED, EQUITY, OPTION, CRYPTO, FOREX, CURRENCY, FUTURES, INDEX, EVENT_CONTRACT, CRYPTO_PERPETUAL, INSTRUMENT_TYPE_FIAT, OPTION_CHAIN};
    }

    @JvmStatic
    public static final InstrumentType fromValue(int i) {
        return INSTANCE.fromValue(i);
    }

    public static EnumEntries<InstrumentType> getEntries() {
        return $ENTRIES;
    }

    private InstrumentType(String str, int i, int i2) {
        this.value = i2;
    }

    @Override // com.squareup.wire.WireEnum
    public int getValue() {
        return this.value;
    }

    static {
        final InstrumentType instrumentType = new InstrumentType("INSTRUMENT_TYPE_UNSPECIFIED", 0, 0);
        INSTRUMENT_TYPE_UNSPECIFIED = instrumentType;
        EQUITY = new InstrumentType("EQUITY", 1, 1);
        OPTION = new InstrumentType("OPTION", 2, 2);
        CRYPTO = new InstrumentType("CRYPTO", 3, 3);
        FOREX = new InstrumentType("FOREX", 4, 4);
        CURRENCY = new InstrumentType("CURRENCY", 5, 5);
        FUTURES = new InstrumentType("FUTURES", 6, 6);
        INDEX = new InstrumentType("INDEX", 7, 7);
        EVENT_CONTRACT = new InstrumentType("EVENT_CONTRACT", 8, 8);
        CRYPTO_PERPETUAL = new InstrumentType("CRYPTO_PERPETUAL", 9, 9);
        INSTRUMENT_TYPE_FIAT = new InstrumentType("INSTRUMENT_TYPE_FIAT", 10, 10);
        OPTION_CHAIN = new InstrumentType("OPTION_CHAIN", 11, 11);
        InstrumentType[] instrumentTypeArr$values = $values();
        $VALUES = instrumentTypeArr$values;
        $ENTRIES = EnumEntries2.enumEntries(instrumentTypeArr$values);
        INSTANCE = new Companion(null);
        final KClass orCreateKotlinClass = Reflection.getOrCreateKotlinClass(InstrumentType.class);
        final Syntax syntax = Syntax.PROTO_3;
        ADAPTER = new EnumAdapter<InstrumentType>(orCreateKotlinClass, syntax, instrumentType) { // from class: rosetta.mainst.InstrumentType$Companion$ADAPTER$1
            /* JADX INFO: Access modifiers changed from: protected */
            @Override // com.squareup.wire.EnumAdapter
            public InstrumentType fromValue(int value) {
                return InstrumentType.INSTANCE.fromValue(value);
            }
        };
    }

    /* compiled from: InstrumentType.kt */
    @Metadata(m3635d1 = {"\u0000\u001e\n\u0002\u0018\u0002\n\u0002\u0010\u0000\n\u0002\b\u0003\n\u0002\u0018\u0002\n\u0002\u0018\u0002\n\u0002\b\u0002\n\u0002\u0010\b\n\u0000\b\u0086\u0003\u0018\u00002\u00020\u0001B\t\b\u0002¢\u0006\u0004\b\u0002\u0010\u0003J\u0012\u0010\u0007\u001a\u0004\u0018\u00010\u00062\u0006\u0010\b\u001a\u00020\tH\u0007R\u0016\u0010\u0004\u001a\b\u0012\u0004\u0012\u00020\u00060\u00058\u0006X\u0087\u0004¢\u0006\u0002\n\u0000¨\u0006\n"}, m3636d2 = {"Lrosetta/mainst/InstrumentType$Companion;", "", "<init>", "()V", "ADAPTER", "Lcom/squareup/wire/ProtoAdapter;", "Lrosetta/mainst/InstrumentType;", "fromValue", "value", "", "rosetta.mainst_externalRelease"}, m3637k = 1, m3638mv = {2, 1, 0}, m3640xi = 48)
    public static final class Companion {
        public /* synthetic */ Companion(DefaultConstructorMarker defaultConstructorMarker) {
            this();
        }

        private Companion() {
        }

        @JvmStatic
        public final InstrumentType fromValue(int value) {
            switch (value) {
                case 0:
                    return InstrumentType.INSTRUMENT_TYPE_UNSPECIFIED;
                case 1:
                    return InstrumentType.EQUITY;
                case 2:
                    return InstrumentType.OPTION;
                case 3:
                    return InstrumentType.CRYPTO;
                case 4:
                    return InstrumentType.FOREX;
                case 5:
                    return InstrumentType.CURRENCY;
                case 6:
                    return InstrumentType.FUTURES;
                case 7:
                    return InstrumentType.INDEX;
                case 8:
                    return InstrumentType.EVENT_CONTRACT;
                case 9:
                    return InstrumentType.CRYPTO_PERPETUAL;
                case 10:
                    return InstrumentType.INSTRUMENT_TYPE_FIAT;
                case 11:
                    return InstrumentType.OPTION_CHAIN;
                default:
                    return null;
            }
        }
    }

    public static InstrumentType valueOf(String str) {
        return (InstrumentType) Enum.valueOf(InstrumentType.class, str);
    }

    public static InstrumentType[] values() {
        return (InstrumentType[]) $VALUES.clone();
    }
}
