...
public final class MemoryCacheService {
    ...
    private final boolean isSizeValid(ImageRequest request, MemoryCache.Key cacheKey, MemoryCache.Value cacheValue, Size size, Scale scale) {
        ...
        if (allowInexactSize) {
            ...
            if (Utils.isMinOrMax(i2) || Math.abs(i2 - height) <= z2) {
                return z2;
            }
        }
    ...
    public final ImageResult3 newResult(Interceptor.Chain chain, ImageRequest request, MemoryCache.Key cacheKey, MemoryCache.Value cacheValue) {
        return new ImageResult3(new BitmapDrawable(request.getContext().getResources(), cacheValue.getBitmap()), request, DataSource.MEMORY_CACHE, cacheKey, getDiskCacheKey(cacheValue), isSampled(cacheValue), Utils.isPlaceholderCached(chain));
    }
