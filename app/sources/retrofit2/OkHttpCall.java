...
final class OkHttpCall<T> implements Call<T> {
    ...
    Response<T> parseResponse(okhttp3.Response response) throws IOException {
        ...
        okhttp3.Response responseBuild = response.newBuilder().body(new NoContentResponseBody(body.get$contentType(), body.getContentLength())).build();
        int code = responseBuild.getCode();
        if (code < 200 || code >= 300) {
            try {
                return Response.error(Utils.buffer(body), responseBuild);
            } finally {
                body.close();
            }
        }
