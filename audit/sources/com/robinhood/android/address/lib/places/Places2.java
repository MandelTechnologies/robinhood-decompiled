...
public final class Places2 {
    ...
    public static final String getState(Place place) {
        ...
        return getComponent$default(place, "administrative_area_level_1", false, 2, null);
    }
    ...
    public static final Place.Builder buildUpon(Place place) {
        Intrinsics.checkNotNullParameter(place, "<this>");
        Place.Builder websiteUri = Place.builder().setAddress(place.getAddress()).setAddressComponents(place.getAddressComponents()).setAttributions(place.getAttributions()).setBusinessStatus(place.getBusinessStatus()).setIconBackgroundColor(place.getIconBackgroundColor()).setIconUrl(place.getIconUrl()).setId(place.getId()).setLatLng(place.getLatLng()).setName(place.getName()).setOpeningHours(place.getOpeningHours()).setPhoneNumber(place.getPhoneNumber()).setPhotoMetadatas(place.getPhotoMetadatas()).setPlusCode(place.getPlusCode()).setPriceLevel(place.getPriceLevel()).setRating(place.getRating()).setPlaceTypes(place.getPlaceTypes()).setUserRatingsTotal(place.getUserRatingsTotal()).setUtcOffsetMinutes(place.getUtcOffsetMinutes()).setViewport(place.getViewport()).setWebsiteUri(place.getWebsiteUri());
    ...
    public static final Place.Builder setStreetNumber(Place.Builder builder, String streetNumber) {
        List<AddressComponent> listEmptyList;
        Intrinsics.checkNotNullParameter(builder, "<this>");
        Intrinsics.checkNotNullParameter(streetNumber, "streetNumber");
        AddressComponents addressComponents = builder.getAddressComponents();
        AddressComponent addressComponentBuild = AddressComponent.builder(streetNumber, CollectionsKt.listOf("street_number")).build();
        if (addressComponents == null || (listEmptyList = addressComponents.asList()) == null) {
            listEmptyList = null;
        } else {
            listEmptyList.add(addressComponentBuild);
        }
        if (listEmptyList == null) {
            listEmptyList = CollectionsKt.emptyList();
        }
        Place.Builder addressComponents2 = builder.setAddressComponents(AddressComponents.newInstance(listEmptyList));
    ...
    public static final boolean isUk(Place place) {
        Intrinsics.checkNotNullParameter(place, "<this>");
        return StringsKt.equals(UNITED_KINGDOM, getCountry(place), true);
    }
}
